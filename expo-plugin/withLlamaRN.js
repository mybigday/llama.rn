 ...
 * @property {string|string[]}  [entitlementsProfile='production'] EAS build profile name to enable entitlements for
...
function withLlamaRn(config, options = {}) {
    const {
        ...
        entitlementsProfile = 'production', // also accepts string array
        forceCxx20 = true,
        enableOpenCL = true, 
    } = options

    // Conditionally add iOS entitlements
    const isProdProfile =
        process.env.EAS_BUILD_PROFILE === entitlementsProfile ||
        process.env.NODE_ENV === 'production' ||
        Array.isArray(entitlementsProfile) && entitlementsProfile.includes(process.env.EAS_BUILD_PROFILE)
    ...
    // Enforce C++20 for app target and all Pods
    if (forceCxx20) {
    ...
    //   Add Android <uses-native-library> for OpenCL
    if (enableOpenCL) {
