...
[![Actions Status](https://github.com/sbhjt-gr/inferra-inferra-llama/workflows/CI/badge.svg)](https://github.com/sbhjt-gr/inferra-inferra-llama/actions)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![npm](https://img.shields.io/npm/v/inferra-llama.svg)](https://www.npmjs.com/package/inferra-llama/)
...
By default, `inferra-llama` will use pre-built `rnllama.xcframework` for iOS. If you want to build from source, please set `RNLLAMA_BUILD_FROM_SOURCE` to `1` in your Podfile.
...
By default, `inferra-llama` will use pre-built libraries for Android. If you want to build from source, please set `rnllamaBuildFromSource` to `true` in `android/gradle.properties`.
...
> **💡 New!** `inferra-llama` now supports **multimodal models** with vision and audio capabilities! See the [Multimodal section](#multimodal-vision--audio) for details.

Load model info only:

```js
import { loadLlamaModelInfo } from 'inferra-llama'
...
```js
import { initLlama } from 'inferra-llama'
...
`inferra-llama` supports multimodal capabilities including vision (images) and audio processing. This allows you to interact with models that can understand both text and media content.
...
`inferra-llama` has universal tool call support by using [minja](https://github.com/google/minja) (as Jinja template parser) and [chat.cpp](https://github.com/ggerganov/llama.cpp/blob/master/common/chat.cpp) in llama.cpp.
...
`inferra-llama` provided a built-in function to convert JSON Schema to GBNF:
...
We have provided a mock version of `inferra-llama` for testing purpose you can use on Jest:

```js
jest.mock('inferra-llama', () => require('inferra-llama/jest/mock'))
```
