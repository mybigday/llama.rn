...
if (!NativeModules.RNLlama) {
  ...
  const benchJson = '{"n_kv_max":2048,"n_batch":2048,"n_ubatch":512,"flash_attn":0,"is_pp_shared":0,"n_gpu_layers":99,"n_threads":8,"n_threads_batch":8,"pp":128,"tg":128,"pl":1,"n_kv":256,"t_pp":0.23381,"speed_pp":547.453064,"t_tg":3.503684,"speed_tg":36.532974,"t":3.737494,"speed":68.495094}'
  ...
  const ensureJSIFunctions = () => {
    ...
    setGlobal('llamaGetBackendDevicesInfo', jest.fn(async () => '[]'))
    setGlobal('llamaLoadSession', jest.fn(async () => ({ tokens_loaded: 0, prompt: '' })))
    setGlobal('llamaSaveSession', jest.fn(async () => 0))
    setGlobal('llamaTokenize', jest.fn(async () => ({ tokens: [], has_media: false, bitmap_hashes: [], chunk_pos: [], chunk_pos_media: [] })))
    setGlobal('llamaDetokenize', jest.fn(async () => ''))
    setGlobal('llamaGetFormattedChat', jest.fn(async () => ({ type: 'llama-chat', prompt: '', has_media: false, media_paths: [] })))
    setGlobal('llamaEmbedding', jest.fn(async () => ({ embedding: demoEmbedding })))
    setGlobal('llamaRerank', jest.fn(async () => [
      { score: 0.9, index: 0 },
      { score: 0.5, index: 1 },
      { score: 0.2, index: 2 },
    ]))
    setGlobal('llamaBench', jest.fn(async () => benchJson))
    setGlobal('llamaCompletion', jest.fn(async (_ctx, _params, onToken) => {
      if (typeof onToken === 'function') {
        tokenEvents.forEach((event) => onToken({ ...event }))
      }
      return { ...completionResult }
    }))
    setGlobal('llamaStopCompletion', jest.fn(async () => {}))
    setGlobal('llamaApplyLoraAdapters', jest.fn(async () => {}))
    setGlobal('llamaRemoveLoraAdapters', jest.fn(async () => {}))
    setGlobal('llamaGetLoadedLoraAdapters', jest.fn(async () => []))
    setGlobal('llamaInitMultimodal', jest.fn(async (contextId) => {
      multimodalMap[contextId] = true
      return true
    }))
    setGlobal('llamaIsMultimodalEnabled', jest.fn(async (contextId) => !!multimodalMap[contextId]))
    setGlobal('llamaGetMultimodalSupport', jest.fn(async () => ({ vision: true, audio: true })))
    setGlobal('llamaReleaseMultimodal', jest.fn(async (contextId) => {
      delete multimodalMap[contextId]
    }))
    setGlobal('llamaInitVocoder', jest.fn(async (contextId) => {
      vocoderMap[contextId] = true
      return true
    }))
    setGlobal('llamaIsVocoderEnabled', jest.fn(async (contextId) => !!vocoderMap[contextId]))
    setGlobal('llamaGetFormattedAudioCompletion', jest.fn(async () => ({ prompt: '', grammar: '' })))
    setGlobal('llamaGetAudioCompletionGuideTokens', jest.fn(async () => []))
    setGlobal('llamaDecodeAudioTokens', jest.fn(async () => []))
    setGlobal('llamaReleaseVocoder', jest.fn(async (contextId) => {
      delete vocoderMap[contextId]
    }))
    setGlobal('llamaEnableParallelMode', jest.fn(async () => true))
    setGlobal('llamaQueueCompletion', jest.fn(async (_ctx, _params, onToken, onComplete) => {
      const reqId = getNextRequestId()
      if (typeof onToken === 'function') {
        tokenEvents.forEach((event) => onToken({ ...event }, reqId))
      }
      if (typeof onComplete === 'function') onComplete({ ...completionResult })
      return { requestId: reqId }
    }))
    setGlobal('llamaCancelRequest', jest.fn(async () => {}))
    setGlobal('llamaQueueEmbedding', jest.fn(async (_ctx, _text, _params, onResult) => {
      const reqId = getNextRequestId()
      if (typeof onResult === 'function') onResult([...demoEmbedding])
      return { requestId: reqId }
    }))
    setGlobal('llamaQueueRerank', jest.fn(async (_ctx, _query, documents, _params, onResult) => {
      const reqId = getNextRequestId()
      const results = (documents || []).map((_, index) => ({ index, score: 1 - index * 0.1 }))
    ...
    setGlobal('llamaSetContextLimit', jest.fn(async () => {}))
  }
  ...
  NativeModules.RNLlama = {
    ...
    queueCompletion: jest.fn(async () => {
      ...
      return { requestId, result: testResult }
    }),

    cancelRequest: jest.fn(async () => {}),

    queueEmbedding: jest.fn(async () => {
      const requestId = getNextRequestId()
      return { requestId, embedding: demoEmbedding }
    }),

    queueRerank: jest.fn(async (_contextId, _query, documents) => {
      const requestId = getNextRequestId()
