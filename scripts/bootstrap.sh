...
# Mirror ggml-hexagon (CMakeLists.txt included: it builds the HTP components)
# and drop files upstream has removed, but keep htp/v73/, which holds the
# FastRPC stub generated by scripts/build-hexagon-htp.sh.
mkdir -p ./cpp/ggml-hexagon
rsync -a --delete --exclude 'htp/v73/' ./$LLAMA_DIR/ggml/src/ggml-hexagon/ ./cpp/ggml-hexagon/
...
# Upstream sources include nlohmann as <nlohmann/json.hpp>. The prebuilt
# xcframework ships the header under Headers/nlohmann, so rewrite the include
# to a quoted form that resolves relative to the framework headers as well.
files_fix_nlohmann_include=(
  ./cpp/*.h
  ./cpp/*.cpp
  ./cpp/common/*.h
  ./cpp/common/*.cpp
...
for file in "${files_fix_nlohmann_include[@]}"; do
  [ -f "$file" ] || continue
  if [ "$OS" = "Darwin" ]; then
