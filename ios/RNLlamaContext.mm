...
//                LOG_WRN("Not preserved because more than 1 token (wrong chat template override?): %s\n", [token UTF8String]);
    ...
    if (params[@"guide_tokens"] && [params[@"guide_tokens"] isKindOfClass:[NSArray class]]) {
        ...
        if (llama->tts_ctx != nullptr) {
            llama->tts_ctx->setGuideTokens(guide_tokens);
        }
    }
    ...
    if (llama->isVocoderEnabled() && llama->tts_ctx != nullptr && !llama->tts_ctx->audio_tokens.empty()) {
        NSMutableArray *audioTokens = [[NSMutableArray alloc] init];
        for (llama_token token : llama->tts_ctx->audio_tokens) {
...
- (NSDictionary *)getFormattedAudioCompletion:(NSString *)speakerJsonStr textToSpeak:(NSString *)textToSpeak {
    ...
    try {
        auto audio_result = llama->tts_ctx->getFormattedAudioCompletion(llama, speakerStr, [textToSpeak UTF8String]);
        return @{
...
- (NSArray *)getAudioCompletionGuideTokens:(NSString *)textToSpeak {
    std::vector<llama_token> guide_tokens = llama->tts_ctx->getAudioCompletionGuideTokens(llama, [textToSpeak UTF8String]);
    NSMutableArray *result = [[NSMutableArray alloc] init];
...
- (NSArray *)decodeAudioTokens:(NSArray *)tokens {
    ...
    std::vector<float> audio_data = llama->tts_ctx->decodeAudioTokens(llama, token_vector);
    NSMutableArray *result = [[NSMutableArray alloc] init];
