...
static void lm_ggml_compute_forward_add_f16_f16(
    ...
    if (nb10 == sizeof(lm_ggml_fp16_t)) {
        for (int ir = ir0; ir < ir1; ++ir) {
            ...
            lm_ggml_fp16_t * src0_ptr = (lm_ggml_fp16_t *) ((char *) src0->data + i03*nb03 + i02*nb02 + i01*nb01);
            lm_ggml_fp16_t * src1_ptr = (lm_ggml_fp16_t *) ((char *) src1->data + i13*nb13 + i12*nb12 + i11*nb11);

            for (int64_t r = 0; r < nr0; ++r) {
                lm_ggml_vec_add_f16(ne10, dst_ptr + r*ne10, src0_ptr + r*ne10, src1_ptr);
            }
        }
    }
    else {
...
static void lm_ggml_compute_forward_sub_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    if (nb10 == sizeof(lm_ggml_fp16_t)) {
        for (int ir = ir0; ir < ir1; ++ir) {
            ...
            for (int64_t r = 0; r < nr0; ++r) {
                lm_ggml_vec_sub_f16(ne10, dst_ptr + r*ne10, src0_ptr + r*ne10, src1_ptr);
            }
        }
    } else {
        // src1 is not contiguous
        LM_GGML_ABORT("unimplemented error");
    }
}
...
static void lm_ggml_compute_forward_sub(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sub_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_mul_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    if (nb10 == sizeof(lm_ggml_fp16_t)) {
        for (int64_t ir = ith; ir < nr; ir += nth) {
            ...
            for (int64_t r = 0 ; r < nr0; ++r) {
                lm_ggml_vec_mul_f16(ne10, dst_ptr + r*ne10, src0_ptr + r*ne10, src1_ptr);
            }
        }
    } else {
        // src1 is not contiguous
...
static void lm_ggml_compute_forward_mul(
    ...
    LM_GGML_ASSERT((src1->type == LM_GGML_TYPE_F32 || src1->type == LM_GGML_TYPE_F16) && "only f32/f16 src1 supported for now");
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_mul_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_div_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    if (nb10 == sizeof(lm_ggml_fp16_t)) {
        for (int64_t ir = ith; ir < nr; ir += nth) {
            ...
            for (int64_t r = 0; r < nr0; ++r) {
                lm_ggml_vec_div_f16(ne10, dst_ptr + r*ne10, src0_ptr + r*ne10, src1_ptr);
            }
        }
    } else {
        // src1 is not contiguous
...
static void lm_ggml_compute_forward_div(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_div_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_sqr_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    for (int i = 0; i < n; i++) {
        lm_ggml_vec_sqr_f16(nc,
                (lm_ggml_fp16_t *) ((char *) dst->data  + i*( dst->nb[1])),
                (lm_ggml_fp16_t *) ((char *) src0->data + i*(src0->nb[1])));
    }
}
...
static void lm_ggml_compute_forward_sqr(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sqr_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_sqrt_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_sqrt(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sqrt_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_log_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT( dst->nb[0] == sizeof(lm_ggml_fp16_t));
    LM_GGML_ASSERT(src0->nb[0] == sizeof(lm_ggml_fp16_t));

    for (int i = 0; i < n; i++) {
        lm_ggml_vec_log_f16(nc,
                (lm_ggml_fp16_t *) ((char *) dst->data  + i*( dst->nb[1])),
                (lm_ggml_fp16_t *) ((char *) src0->data + i*(src0->nb[1])));
    }
}
...
static void lm_ggml_compute_forward_log(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_log_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_sin_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_sin(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sin_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_cos_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_cos(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_cos_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_abs_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_abs(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_abs_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_sgn_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_sgn(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sgn_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_neg_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_neg(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_neg_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_step_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_step(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_step_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_tanh_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_tanh(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_tanh_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_elu_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_elu(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_elu_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_relu_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_relu(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_relu_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_sigmoid_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_sigmoid(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_sigmoid_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_hardswish_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_hardswish(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_hardswish_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_hardsigmoid_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
    ...
    for (int i = 0; i < n; i++) {
        lm_ggml_vec_hardsigmoid_f16(nc,
                (lm_ggml_fp16_t *) ((char *) dst->data  + i*( dst->nb[1])),
                (lm_ggml_fp16_t *) ((char *) src0->data + i*(src0->nb[1])));
    }
}
...
static void lm_ggml_compute_forward_hardsigmoid(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_hardsigmoid_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_exp_f16(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_exp(
    ...
    switch (src0->type) {
        ...
        case LM_GGML_TYPE_F16:
            {
                lm_ggml_compute_forward_exp_f16(params, dst);
            } break;
        default:
            {
                LM_GGML_ABORT("fatal error");
...
static void lm_ggml_compute_forward_l2_norm(
    const struct lm_ggml_compute_params * params,
    struct lm_ggml_tensor * dst) {
...
static void lm_ggml_compute_forward_rwkv_wkv7(
        const struct lm_ggml_compute_params * params,
        struct lm_ggml_tensor * dst) {
