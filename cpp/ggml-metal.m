...
enum lm_ggml_metal_kernel_type {
    ...
    LM_GGML_METAL_KERNEL_TYPE_SOFT_MAX,
    LM_GGML_METAL_KERNEL_TYPE_SOFT_MAX_4,
...
#define LM_GGML_METAL_ADD_KERNEL(e, name, supported) \
        if (supported) { \
            ...
            LM_GGML_METAL_LOG_WARN("%s: skipping %-32s (not supported)\n", __func__, "kernel_"#name); \
        }
        ...
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SOFT_MAX,                  soft_max,               ctx->support_simdgroup_reduction);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SOFT_MAX_4,                soft_max_4,             ctx->support_simdgroup_reduction);
                ...
                switch (dst->op) {
                    ...
                    case LM_GGML_OP_UNARY:
                        switch (lm_ggml_get_unary_op(gf->nodes[i])) {
                            ...
                            case LM_GGML_UNARY_OP_GELU:
                                {
                                    id<MTLComputePipelineState> pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_GELU].pipeline;
                            ...
                            case LM_GGML_UNARY_OP_GELU_QUICK:
                                {
                                    id<MTLComputePipelineState> pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_GELU_QUICK].pipeline;
                            ...
                            case LM_GGML_UNARY_OP_SILU:
                                {
                                    id<MTLComputePipelineState> pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_SILU].pipeline;
                    ...
                    case LM_GGML_OP_SOFT_MAX:
                        {
                            ...
                            if (ne00%4 == 0) {
                                ...
                                pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_SOFT_MAX_4].pipeline;
                            } else {
                    ...
                    case LM_GGML_OP_MUL_MAT_ID:
                        {
                            ...
                            if ([ctx->device supportsFamily:MTLGPUFamilyApple7] &&
                                ...
                                if (lm_ggml_is_quantized(src2t)) {
                                    LM_GGML_ASSERT(ne20 >= nth0*nth1);
                                }
                    ...
                    case LM_GGML_OP_ALIBI:
                        {
                            ...
                            id<MTLComputePipelineState> pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_ALIBI_F32].pipeline;
                    ...
                    case LM_GGML_OP_ARGSORT:
                        {
                            ...
                            switch (order) {
                                case LM_GGML_SORT_ASC:  pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_ARGSORT_F32_I32_ASC].pipeline;  break;
                                case LM_GGML_SORT_DESC: pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_ARGSORT_F32_I32_DESC].pipeline; break;
                                default: LM_GGML_ASSERT(false);
                            };
...
static lm_ggml_backend_buffer_t lm_ggml_backend_metal_buffer_type_alloc_buffer(lm_ggml_backend_buffer_type_t buft, size_t size) {
    ...
    LM_GGML_METAL_LOG_INFO("%s: allocated buffer, size = %8.2f MiB", __func__, size_aligned / 1024.0 / 1024.0);
...
lm_ggml_backend_buffer_t lm_ggml_backend_metal_buffer_from_ptr(void * data, size_t size, size_t max_size) {
    ...
    if (size_aligned <= device.maxBufferLength) {
        ...
        for (size_t i = 0; i < size; i += size_step) {
            ...
            LM_GGML_METAL_LOG_INFO("%s: allocated buffer, size = %8.2f MiB, offs = %12ld", __func__, size_step_aligned / 1024.0 / 1024.0, i);
            if (i + size_step < size) {
                LM_GGML_METAL_LOG_INFO("\n");
            }
...
static bool lm_ggml_backend_metal_graph_compute(lm_ggml_backend_t backend, struct lm_ggml_cgraph * cgraph) {
