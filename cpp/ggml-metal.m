...
static id<MTLDevice> lm_ggml_backend_metal_device_acq(struct lm_ggml_backend_metal_device_context * ctx) {
    ...
    if (ctx->mtl_device == nil) {
        ...
        if (ctx->mtl_device) {
            ...
            ctx->use_concurrency = getenv("LM_GGML_METAL_CONCURRENCY_DISABLE") == nil;

            {
                const char * val = getenv("LM_GGML_METAL_GRAPH_DEBUG");
            ...
            if (getenv("LM_GGML_METAL_GRAPH_OPTIMIZE_DISABLE") != NULL) {
...
@interface lm_ggml_metal_kernel_wrapper : NSObject

@property (nonatomic, assign) struct lm_ggml_metal_kernel kernel;

@end

@implementation lm_ggml_metal_kernel_wrapper
- (void) dealloc {
...
enum lm_ggml_metal_kernel_type {
  ...
  //LM_GGML_METAL_KERNEL_TYPE_MUL_MV_ID_F16_F16,
    ...
    LM_GGML_METAL_KERNEL_TYPE_CPY_F32_I32,
    LM_GGML_METAL_KERNEL_TYPE_CPY_I32_F32,
...
struct lm_ggml_metal_command_buffer {
    ...
    struct lm_ggml_mem_ranges * mem_ranges;
};
...
static struct lm_ggml_backend_metal_context * lm_ggml_metal_init(lm_ggml_backend_dev_t dev) {
    ...
    for (int i = 0; i < LM_GGML_METAL_MAX_COMMAND_BUFFERS; ++i) {
        ...
        if (ctx_dev->use_concurrency) {
            ctx->cmd_bufs[i].mem_ranges = lm_ggml_mem_ranges_init(ctx_dev->debug_graph);
        }
    }
...
#define LM_GGML_METAL_ADD_KERNEL(e, name, supported) \
        ...
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_NORM,                            norm,                            has_simdgroup_reduction);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SSM_CONV_F32,                    ssm_conv_f32,                    true);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SSM_SCAN_F32,                    ssm_scan_f32,                    has_simdgroup_reduction);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SSM_SCAN_F32_GROUP,              ssm_scan_f32_group,              has_simdgroup_reduction);
      ...
      //LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_MUL_MV_ID_F16_F16,               mul_mv_id_f16_f16,               has_simdgroup_reduction);
        ...
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_CPY_F32_I32,                     cpy_f32_i32,                     true);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_CPY_I32_F32,                     cpy_i32_f32,                     true);
        ...
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_SUM_ROWS,                        sum_rows,                        has_simdgroup_reduction);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_MEAN,                            mean,                            has_simdgroup_reduction);
        LM_GGML_METAL_ADD_KERNEL(LM_GGML_METAL_KERNEL_TYPE_ARGMAX,                          argmax,                          has_simdgroup_reduction);
...
static id<MTLComputePipelineState> lm_ggml_metal_get_kernel(struct lm_ggml_backend_metal_context * ctx, const char * name) {
    NSString * key = [NSString stringWithUTF8String:name];

    lm_ggml_metal_kernel_wrapper * obj = [ctx->kernels_ext objectForKey:key];
    if (obj) {
...
static id<MTLComputePipelineState> lm_ggml_metal_compile_kernel(lm_ggml_backend_t backend, const char * base, const char * name, MTLFunctionConstantValues * cv) {
    ...
    @autoreleasepool {
        ...
        LM_GGML_LOG_DEBUG("%s: compiling kernel: base = '%s', name = '%s'\n", __func__, base, name);
        ...
        struct lm_ggml_metal_kernel kernel = {
            /*.pipeline =*/ [ctx_dev->mtl_device newComputePipelineStateWithFunction:metal_function error:&error],
        };

        lm_ggml_metal_kernel_wrapper * obj = [[lm_ggml_metal_kernel_wrapper alloc] init];
        obj.kernel = kernel;
...
// tokens per expert
static size_t lm_ggml_metal_mul_mat_id_extra_tpe(const struct lm_ggml_tensor * op) {
...
static size_t lm_ggml_metal_mul_mat_id_extra_ids(const struct lm_ggml_tensor * op) {
    assert(op->op == LM_GGML_OP_MUL_MAT_ID);
...
static bool lm_ggml_metal_flash_attn_ext_use_vec(const struct lm_ggml_tensor * op) {
...
static size_t lm_ggml_metal_flash_attn_ext_extra_tmp(const struct lm_ggml_tensor * op) {
    assert(op->op == LM_GGML_OP_FLASH_ATTN_EXT);
...
static id<MTLComputePipelineState> lm_ggml_metal_get_pipeline_flash_attn_ext(
        lm_ggml_backend_t backend, struct lm_ggml_tensor * op,
        bool    has_mask,
        bool    has_sinks,
        bool    has_bias,
        bool    has_scap,
    ...
    @autoreleasepool {
        ...
        id<MTLComputePipelineState> res = lm_ggml_metal_get_kernel(ctx, name);
        if (res) {
            // kernel found
            return res;
        }
        ...
        res = lm_ggml_metal_compile_kernel(backend, base, name, cv);
...
static id<MTLComputePipelineState> lm_ggml_metal_get_pipeline_flash_attn_ext_vec(
        lm_ggml_backend_t backend, struct lm_ggml_tensor * op,
        bool    has_mask,
        bool    has_sinks,
        bool    has_bias,
...
static id<MTLComputePipelineState> lm_ggml_metal_get_pipeline_flash_attn_ext_vec_reduce(
        lm_ggml_backend_t backend, struct lm_ggml_tensor * op,
        int32_t dv,
        int32_t nwg) {
    struct lm_ggml_backend_metal_context * ctx = backend->context;
...
static id<MTLComputePipelineState> lm_ggml_metal_get_pipeline_bin(
        lm_ggml_backend_t backend, enum lm_ggml_op op,
        int32_t n_fuse,
        bool row) {
    struct lm_ggml_backend_metal_context * ctx = backend->context;
    ...
    @autoreleasepool {
        ...
        return lm_ggml_metal_compile_kernel(backend, base, name, nil);
    }
}

static id<MTLComputePipelineState> lm_ggml_metal_get_pipeline_rms_norm(
        lm_ggml_backend_t backend, struct lm_ggml_tensor * op,
        int32_t n_fuse) {
    struct lm_ggml_backend_metal_context * ctx = backend->context;
...
static void lm_ggml_metal_free(struct lm_ggml_backend_metal_context * ctx) {
    ...
    for (int i = 0; i < LM_GGML_METAL_MAX_COMMAND_BUFFERS; ++i) {
        ...
        if (ctx->cmd_bufs[i].mem_ranges) {
            lm_ggml_mem_ranges_free(ctx->cmd_bufs[i].mem_ranges);
        }
    }
...
//
static id<MTLBuffer> lm_ggml_metal_get_buffer(const struct lm_ggml_tensor * t, size_t * offs) {
...
struct lm_ggml_metal_encode_context {
...
static bool lm_ggml_metal_encode_concurrency_reset(struct lm_ggml_metal_encode_context * ctx) {
    if (!ctx->mem_ranges) {
        return true;
    }
...
static bool lm_ggml_metal_encode_concurrency_check(struct lm_ggml_metal_encode_context * ctx, const struct lm_ggml_tensor * node) {
    if (!ctx->mem_ranges) {
        return false;
    }
...
static bool lm_ggml_metal_encode_concurrency_add(struct lm_ggml_metal_encode_context * ctx, const struct lm_ggml_tensor * node) {
    if (!ctx->mem_ranges) {
        return true;
    }
...
static int lm_ggml_metal_encode_node(struct lm_ggml_metal_encode_context * ctx_enc, int idx, int idx_end) {
    lm_ggml_backend_t backend = ctx_enc->backend;
    ...
    size_t offs_src[LM_GGML_MAX_SRC];

    id<MTLBuffer> id_src[LM_GGML_MAX_SRC];

    enum lm_ggml_type srct[LM_GGML_MAX_SRC];

    for (int i = 0; i < LM_GGML_MAX_SRC; i++) {
        offs_src[i] = 0;
        id_src[i] = node->src[i] ? lm_ggml_metal_get_buffer(node->src[i], &offs_src[i]) : nil;
        srct[i]   = node->src[i] ? node->src[i]->type : LM_GGML_TYPE_COUNT;
    }
    ...
    //
    {
        const bool is_concurrent = lm_ggml_metal_encode_concurrency_check(ctx_enc, node);

        if (!is_concurrent) {
            lm_ggml_metal_encode_concurrency_reset(ctx_enc);
        }

        if (ctx_dev->debug_graph > 0) {
            LM_GGML_LOG_DEBUG("%s: node[%5d] - %-12s %s\n", __func__, idx, lm_ggml_op_name(dst->op), is_concurrent ? "(concurrent)" : "");
        }
        if (ctx_dev->debug_graph > 1) {
            ...
            if (dst) {
                LM_GGML_LOG_DEBUG("%s: dst  - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], 1, %s\n", __func__, lm_ggml_type_name(dstt), ne0, ne1, ne2, ne3, nb0, nb1, nb2, nb3,
                        dst->name);
            }
        }
    }
        ...
        case LM_GGML_OP_DIV:
            {
                ...
                if (lm_ggml_nelements(src1) == ne10 && lm_ggml_is_contiguous(src1) && ne00 % 4 == 0 && ne10 % 4 == 0) {
                    ...
                    pipeline = lm_ggml_metal_get_pipeline_bin(backend, dst->op, n_fuse, true);

                    bcast_row = true;
                } else {
                    pipeline = lm_ggml_metal_get_pipeline_bin(backend, dst->op, n_fuse, false);
                }
                ...
                if (n_fuse > 1) {
                    ...
                    for (int i = 1; i < n_fuse; ++i) {
                        if (!lm_ggml_metal_encode_concurrency_check(ctx_enc, nodes[i])) {
                            lm_ggml_metal_encode_concurrency_reset(ctx_enc);
        ...
        case LM_GGML_OP_ACC:
            {
                ...
                const id<MTLComputePipelineState> pipeline = lm_ggml_metal_get_pipeline_bin(backend, LM_GGML_OP_ADD, 1, false);
        ...
        case LM_GGML_OP_MUL_MAT_ID:
            {
                ...
                if ([device supportsFamily:MTLGPUFamilyApple7] &&
                    ...
                    size_t offs_tpe = offs_dst + lm_ggml_nbytes(dst);
                    size_t offs_ids = offs_tpe + lm_ggml_metal_mul_mat_id_extra_tpe(dst);
        ...
        case LM_GGML_OP_RMS_NORM:
            {
                ...
                const id<MTLComputePipelineState> pipeline = lm_ggml_metal_get_pipeline_rms_norm(backend, node, n_fuse);
        ...
        case LM_GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                if (!lm_ggml_metal_flash_attn_ext_use_vec(dst)) {
                    // half8x8 kernel
...
#define FATTN_SMEM(nsg) (LM_GGML_PAD((nqptg*(ne00 + 2*LM_GGML_PAD(ne20, 64) + 2*(2*ncpsg)) + is_q*(16*32*(nsg)))*(sizeof(float)/2), 16))
                    ...
                    id<MTLComputePipelineState> pipeline = lm_ggml_metal_get_pipeline_flash_attn_ext(backend, node, has_mask, has_sinks, has_bias, has_scap, nsg);
...
#define FATTN_SMEM(nsg) (LM_GGML_PAD((nqptg*(LM_GGML_PAD(ne00, 128) + 4*ncpsg*(nsg)) + 2*LM_GGML_PAD(ne20, 128)*(nsg))*(sizeof(float)/2), 16))
                    ...
                    id<MTLComputePipelineState> pipeline = lm_ggml_metal_get_pipeline_flash_attn_ext_vec(backend, node, has_mask, has_sinks, has_bias, has_scap, nsg, nwg);

                    LM_GGML_ASSERT(nsg*32 <= (int) pipeline.maxTotalThreadsPerThreadgroup);
                    ...
                    if (nwg == 1) {
                        ...
                        const size_t offs_tmp = offs_dst + lm_ggml_nbytes(dst);
                        ...
                        // reduce the results from the workgroups
                        {
                            ...
                            id<MTLComputePipelineState> pipeline0 = lm_ggml_metal_get_pipeline_flash_attn_ext_vec_reduce(backend, node, ne20, nwg);
...
#undef FATTN_SMEM
        ...
        case LM_GGML_OP_CONT:
            {
                ...
                switch (src0t) {
                    case LM_GGML_TYPE_F32:
                        {
                            ...
                            switch (dstt) {
                                ...
                                case LM_GGML_TYPE_I32:    pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_CPY_F32_I32].pipeline; break;
                    ...
                    case LM_GGML_TYPE_I32:
                        {
                            switch (dstt) {
                                case LM_GGML_TYPE_F32: pipeline = ctx->kernels[LM_GGML_METAL_KERNEL_TYPE_CPY_I32_F32].pipeline; break;
                                default: LM_GGML_ABORT("not implemented");
                            };
                        } break;
                    case LM_GGML_TYPE_F16:
    ...
    for (int i = 0; i < n_fuse; ++i) {
        if (!lm_ggml_metal_encode_concurrency_add(ctx_enc, nodes[i])) {
            lm_ggml_metal_encode_concurrency_reset(ctx_enc);
        }
    }
...
static void lm_ggml_backend_metal_buffer_private_memset_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor, uint8_t value, size_t offset, size_t size) {
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> buf_dst = lm_ggml_metal_get_buffer(tensor, &buf_dst_offset);
...
static void lm_ggml_backend_metal_buffer_private_get_tensor(lm_ggml_backend_buffer_t buffer, const struct lm_ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> buf_src = lm_ggml_metal_get_buffer(tensor, &buf_src_offset);
...
static lm_ggml_backend_buffer_t lm_ggml_backend_metal_buffer_type_alloc_buffer(lm_ggml_backend_buffer_type_t buft, size_t size, bool shared) {
    ...
    struct lm_ggml_backend_buffer_i buf_i = ctx->is_shared ? lm_ggml_backend_metal_buffer_shared_i : lm_ggml_backend_metal_buffer_private_i;

    return lm_ggml_backend_buffer_init(buft, buf_i, ctx, size);
}
...
static size_t lm_ggml_backend_metal_buffer_type_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        case LM_GGML_OP_MUL_MAT_ID:
            {
                res += lm_ggml_metal_mul_mat_id_extra_tpe(tensor);
                res += lm_ggml_metal_mul_mat_id_extra_ids(tensor);
            } break;
        case LM_GGML_OP_FLASH_ATTN_EXT:
            {
                if (lm_ggml_metal_flash_attn_ext_use_vec(tensor)) {
                    res += lm_ggml_metal_flash_attn_ext_extra_tmp(tensor);
                }
            } break;
        default:
            break;
...
static void lm_ggml_backend_metal_set_tensor_async(lm_ggml_backend_t backend,       struct lm_ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    @autoreleasepool {
        ...
        if (buf_dst == nil) {
            LM_GGML_ABORT("%s: failed to find buffer for tensor '%s'\n", __func__, tensor->name);
        }
...
static void lm_ggml_backend_metal_graph_optimize(lm_ggml_backend_t backend, struct lm_ggml_cgraph * cgraph) {
    ...
    if (ctx_dev->use_graph_optimize) {
        lm_ggml_metal_graph_optimize(cgraph);
    }

    //printf("%s: graph optimize took %.3f ms\n", __func__, (lm_ggml_time_us() - t_start) / 1000.0);
}
...
static void lm_ggml_backend_metal_set_n_cb(lm_ggml_backend_t backend, int n_cb) {
    ...
    ctx->encode_async = Block_copy(^(size_t iter) {
        ...
        struct lm_ggml_mem_ranges     * mem_ranges = ctx->cmd_bufs[cb_idx].mem_ranges;

        if (mem_ranges) {
            lm_ggml_mem_ranges_reset(mem_ranges);
        }
        ...
        struct lm_ggml_metal_encode_context ctx_enc = {
            /*.backend    =*/ backend,
            /*.encoder    =*/ encoder,
        ...
        for (int idx = node_start; idx < node_end;) {
            ...
            const int res = lm_ggml_metal_encode_node(&ctx_enc, idx, node_end);
            if (idx + res > node_end) {
...
static struct lm_ggml_backend_i lm_ggml_backend_metal_i = {
    ...
    /* .event_record            = */ NULL,
    /* .event_wait              = */ NULL,
    /* .optimize_graph          = */ lm_ggml_backend_metal_graph_optimize,
};
...
static lm_ggml_backend_buffer_type_t lm_ggml_backend_metal_device_get_buffer_type(lm_ggml_backend_dev_t dev) {
    ...
    return ctx_dev->use_shared_buffers ? lm_ggml_backend_metal_buffer_type_shared() : lm_ggml_backend_metal_buffer_type_private();
}
...
static lm_ggml_backend_buffer_t lm_ggml_backend_metal_device_buffer_mapped(lm_ggml_backend_dev_t dev, void * ptr, size_t size, size_t max_tensor_size) {
    ...
    return lm_ggml_backend_buffer_init(lm_ggml_backend_metal_buffer_type_mapped(), lm_ggml_backend_metal_buffer_shared_i, ctx, size);
}
