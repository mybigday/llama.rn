...
int lm_ggml_metal_op_mul_mat(lm_ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == LM_GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        lm_ggml_metal_pipeline_t pipeline = lm_ggml_metal_library_get_pipeline_mul_mv_ext(lib, op->src[0]->type, op->src[1]->type, r1ptg);
        ...
        if (op->src[0]->type == LM_GGML_TYPE_Q8_0) {
