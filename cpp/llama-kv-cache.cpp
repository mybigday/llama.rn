...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](lm_ggml_backend_buffer_type_t buft) -> lm_ggml_context * {
        ...
        if (it == ctx_map.end()) {
            lm_ggml_init_params params = {
                /*.mem_size   =*/ size_t(3u*(1 + n_stream)*n_layer*lm_ggml_tensor_overhead()), //Reserve tensor metadata for up to 3 tensors per layer (K, V, and optional K_idx), plus one view per tensor per stream.
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        lm_ggml_tensor * k_idx = n_embd_k_idx > 0
            ? lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, n_embd_k_idx, kv_size, n_stream)
            : nullptr;
        if (k_idx) {
            lm_ggml_format_name(k_idx, "cache_k_idx_l%d", il);
        ...
        for (uint32_t s = 0; s < n_stream; ++s) {
            k_idx_stream.push_back(k_idx
                ? lm_ggml_view_2d(ctx, k_idx, n_embd_k_idx, kv_size, k_idx->nb[1], s*k_idx->nb[2])
                : nullptr);
        }
...

    {
        ...
        const std::string k_log = format(", K (%s): %7.2f MiB", lm_ggml_type_name(type_k), (float) memory_size_k / mib);
        const std::string v_log = format(", V (%s): %7.2f MiB", lm_ggml_type_name(type_v), (float) memory_size_v / mib);

        std::string k_idx_log;
        if (memory_size_k_idx > 0) {
            k_idx_log = format(", K_idx (%s): %7.2f MiB", lm_ggml_type_name(LM_GGML_TYPE_F32), (float) memory_size_k_idx / mib);
        }
        ...
        for (size_t i = 0; i < n_copy; ++i) {
            ...
            for (uint32_t il = 0; il < layers.size(); ++il) {
                ...
                if (layer.k_idx_stream[ssrc]) {
                    LM_GGML_ASSERT(layer.k_idx_stream[sdst]);
                    lm_ggml_backend_tensor_copy(layer.k_idx_stream[ssrc], layer.k_idx_stream[sdst]);
                }
            }
        }
    }
...
void llama_kv_cache::apply_ubatch(const slot_info & sinfo, const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < LLAMA_MAX_SEQ; ++s) {
        ...
        if (cells.seq_pos_min(s) <= seq_pos_max_rm[s]) {
            ...
            LM_GGML_ASSERT(seq_rm(s, cells.seq_pos_min(s), seq_pos_max_rm[s] + 1));
        }
    }
...
lm_ggml_tensor * llama_kv_cache::get_k_storage(int32_t il) const {
...
lm_ggml_tensor * llama_kv_cache::get_k_idx(lm_ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
    auto * k_idx = layers[ikv].k_idx;
    LM_GGML_ASSERT(k_idx);

    const uint64_t kv_size = get_size();
    const int64_t  n_idx   = k_idx->ne[0];                 // 128
    const uint32_t ns      = sinfo.s1 - sinfo.s0 + 1;

    return lm_ggml_view_4d(ctx, k_idx,
            n_idx, 1, n_kv, ns,
            lm_ggml_row_size(k_idx->type, n_idx),             // nb1 (single head)
            lm_ggml_row_size(k_idx->type, n_idx),             // nb2 (per cell)
            lm_ggml_row_size(k_idx->type, n_idx*kv_size),     // nb3 (per stream)
            lm_ggml_row_size(k_idx->type, n_idx*kv_size)*sinfo.s0);
}
...
lm_ggml_tensor * llama_kv_cache::cpy_k_idx(lm_ggml_context * ctx, lm_ggml_tensor * k_idx_cur, lm_ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    LM_GGML_UNUSED(sinfo);
    const int32_t ikv = map_layer_ids.at(il);
    lm_ggml_tensor * k_idx = layers[ikv].k_idx;
    LM_GGML_ASSERT(k_idx && "cpy_k_idx on a layer with no indexer cache");
    ...
    LM_GGML_ASSERT(lm_ggml_row_size(k_idx_cur->type, n_embd_head) == k_idx_cur->nb[1]);
    k_idx_cur = lm_ggml_view_2d(ctx, k_idx_cur, n_embd_gqa, n_tokens, k_idx_cur->nb[2], 0);

    const int64_t n_stream = k_idx->ne[2];
    if (n_stream > 1) {
        const int64_t kv_size = get_size();
        k_idx = lm_ggml_reshape_2d(ctx, k_idx, n_embd_gqa, kv_size*n_stream);
    }
    return lm_ggml_set_rows(ctx, k_idx, k_idx_cur, k_idxs);   // same k_idxs as the K store
}
...
size_t llama_kv_cache::size_k_idx_bytes() const {
    ...
    for (const auto & layer : layers) {
        if (layer.k_idx) {
            size_k_idx_bytes += lm_ggml_nbytes(layer.k_idx);
        }
    }
...
void llama_kv_cache::state_write_data(llama_io_write_i & io, const cell_ranges_t & cr) const {
    ...
    if (size_k_idx_bytes() > 0) {
        ...
        for (const auto & layer : layers) {
            ...
            LM_GGML_ASSERT(layer.k_idx_stream[cr.strm]);

            const int32_t k_idx_type_i = (int32_t) layer.k_idx->type;
            io.write(&k_idx_type_i, sizeof(k_idx_type_i));

            const uint64_t k_idx_size_row = lm_ggml_row_size(layer.k_idx->type, layer.k_idx->ne[0]);
...
bool llama_kv_cache::state_read_data(llama_io_read_i & io, uint32_t strm, uint32_t cell_count, const slot_info & sinfo) {
    ...
    if (size_k_idx_bytes() > 0) {
        ...
        for (const auto & layer : layers) {
            ...
            LM_GGML_ASSERT(layer.k_idx_stream[strm]);
            ...
            const uint64_t expected_k_idx_size_row = lm_ggml_row_size(layer.k_idx->type, layer.k_idx->ne[0]);
...
lm_ggml_tensor * llama_kv_cache_context::get_k_idx(lm_ggml_context * ctx, int32_t il) const {
...
lm_ggml_tensor * llama_kv_cache_context::cpy_k_idx(lm_ggml_context * ctx, lm_ggml_tensor * k_idx_cur, lm_ggml_tensor * k_idxs, int32_t il) const {
