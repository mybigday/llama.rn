...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](lm_ggml_backend_buffer_type_t buft) -> lm_ggml_context * {
        ...
        if (it == ctx_map.end()) {
            lm_ggml_init_params params = {
                /*.mem_size   =*/ size_t(2u*(1 + n_stream)*n_layer*lm_ggml_tensor_overhead()),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        has_k && lm_ggml_format_name(k, "cache_%sk_l%d", name_tag, il);
        has_v && lm_ggml_format_name(v, "cache_%sv_l%d", name_tag, il);
