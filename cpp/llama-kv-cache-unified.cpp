...
lm_ggml_tensor * llama_kv_cache_unified::cpy_k(lm_ggml_context * ctx, lm_ggml_tensor * k_cur, lm_ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
...
lm_ggml_tensor * llama_kv_cache_unified::cpy_v(lm_ggml_context * ctx, lm_ggml_tensor * v_cur, lm_ggml_tensor * v_idxs, int32_t il, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
    ...
    if (v_idxs && supports_set_rows) {
        ...
        lm_ggml_tensor * v_view = lm_ggml_reshape_3d(ctx, v, 1, v->ne[1], v->ne[0]);

        // note: the V cache is transposed when not using flash attention
        v_cur = lm_ggml_permute(ctx, lm_ggml_reshape_3d(ctx, v_cur, v_cur->ne[0], 1, v_cur->ne[1]), 2, 0, 1, 3);
        ...
        //v_cur = lm_ggml_cont_3d(ctx, v_cur, 1, v_cur->ne[0], v_cur->ne[1]);
...
lm_ggml_tensor * llama_kv_cache_unified::build_input_v_idxs(lm_ggml_context * ctx, const llama_ubatch & ubatch) const {
    ...
    lm_ggml_tensor * v_idxs = lm_ggml_new_tensor_1d(ctx, LM_GGML_TYPE_I64, n_tokens);
...
lm_ggml_tensor * llama_kv_cache_unified_context::cpy_k(lm_ggml_context * ctx, lm_ggml_tensor * k_cur, lm_ggml_tensor * k_idxs, int32_t il) const {
    return kv->cpy_k(ctx, k_cur, k_idxs, il, sinfos[i_cur]);
}

lm_ggml_tensor * llama_kv_cache_unified_context::cpy_v(lm_ggml_context * ctx, lm_ggml_tensor * v_cur, lm_ggml_tensor * v_idxs, int32_t il) const {
    return kv->cpy_v(ctx, v_cur, v_idxs, il, sinfos[i_cur]);
}

lm_ggml_tensor * llama_kv_cache_unified_context::build_input_k_idxs(lm_ggml_context * ctx, const llama_ubatch & ubatch) const {
    return kv->build_input_k_idxs(ctx, ubatch);
}

lm_ggml_tensor * llama_kv_cache_unified_context::build_input_v_idxs(lm_ggml_context * ctx, const llama_ubatch & ubatch) const {
...
void llama_kv_cache_unified_context::set_input_k_idxs(lm_ggml_tensor * dst, const llama_ubatch * ubatch) const {
    kv->set_input_k_idxs(dst, ubatch, sinfos[i_cur]);
}

void llama_kv_cache_unified_context::set_input_v_idxs(lm_ggml_tensor * dst, const llama_ubatch * ubatch) const {
