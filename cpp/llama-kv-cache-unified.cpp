...
void llama_kv_cache_unified::set_input_kq_mask(lm_ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn) const {
    ...
    for (uint32_t h = 0; h < 1; ++h) {
        ...
        // mask padded tokens
        if (data) {
            for (uint32_t j = n_tokens; j < LM_GGML_PAD(n_tokens, LM_GGML_KQ_MASK_PAD); ++j) {
                for (uint32_t i = 0; i < n_kv; ++i) {
...
lm_ggml_tensor * llama_kv_cache_unified_state::get_k(lm_ggml_context * ctx, int32_t il) const {
    return kv->get_k(ctx, il, n_kv);
}

lm_ggml_tensor * llama_kv_cache_unified_state::get_v(lm_ggml_context * ctx, int32_t il) const {
    return kv->get_v(ctx, il, n_kv);
}

lm_ggml_tensor * llama_kv_cache_unified_state::cpy_k(lm_ggml_context * ctx, lm_ggml_tensor * k_cur, int32_t il) const {
    return kv->cpy_k(ctx, k_cur, il, head);
}

lm_ggml_tensor * llama_kv_cache_unified_state::cpy_v(lm_ggml_context * ctx, lm_ggml_tensor * v_cur, int32_t il) const {
    return kv->cpy_v(ctx, v_cur, il, head);
}

void llama_kv_cache_unified_state::set_input_k_shift(lm_ggml_tensor * dst) const {
    kv->set_input_k_shift(dst);
}

void llama_kv_cache_unified_state::set_input_kq_mask(lm_ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn) const {
    kv->set_input_kq_mask(dst, ubatch, causal_attn);
}

void llama_kv_cache_unified_state::set_input_pos_bucket(lm_ggml_tensor * dst, const llama_ubatch * ubatch) const {
