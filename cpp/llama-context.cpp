...
llama_token llama_context::get_sampled_token_ith(int32_t idx) {
    ...
    try {
        ...
        LM_GGML_ASSERT(row < (int64_t) sampling.sampled_size);
...
bool llama_context::set_sampler(llama_seq_id seq_id, llama_sampler * sampler) {
    ...
    if (sampler && can_offload) {
        lm_ggml_backend_buffer_type_t buft = lm_ggml_backend_dev_buffer_type(model.dev_output());
        auto * host_buft = lm_ggml_backend_dev_host_buffer_type(model.dev_output());
        if (host_buft) {
            buft = host_buft;
        }
...
static void copy_tensor_async_ints(
    ...
    for (const auto & [seq_id, tensor] : tensor_map) {
        ...
        LM_GGML_ASSERT(row < sampled_size);
        ...
        lm_ggml_backend_tensor_get_async(backend, tensor, sampled + row, 0, sizeof(sampled[row]));
    }
}
                ...
                case LLAMA_POOLING_TYPE_NONE:
                    {
                        ...
                        if (n_outputs) {
                            LM_GGML_ASSERT( n_outputs_prev + n_outputs <= n_outputs_all);
                            LM_GGML_ASSERT((n_outputs_prev + n_outputs)*n_embd_out <= (int64_t) embd_size);
