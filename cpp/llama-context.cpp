...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    if (embd_pre_norm.data && t_h_pre_norm && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
        lm_ggml_backend_t backend_h = lm_ggml_backend_sched_get_tensor_backend(sched.get(), t_h_pre_norm);
        LM_GGML_ASSERT(backend_h != nullptr);

        const uint32_t n_embd = hparams.n_embd;
        LM_GGML_ASSERT(n_tokens*n_embd <= (int64_t) embd_pre_norm.size);
        lm_ggml_backend_tensor_get_async(backend_h, t_h_pre_norm, embd_pre_norm.data, 0, n_tokens*n_embd*sizeof(float));
    }
    ...
    do {
        ...
        // only meaningful in LLAMA_POOLING_TYPE_NONE (per-token); other pooling modes are ignored.
        {
            ...
            if (embd_pre_norm.data && t_h_pre_norm && n_rows > 0 && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
                ...
                LM_GGML_ASSERT((offset + n_rows)*n_embd <= (int64_t) embd_pre_norm.size);
                lm_ggml_backend_tensor_get_async(backend_h, t_h_pre_norm, embd_pre_norm_out, 0, n_rows*n_embd*sizeof(float));
            }
        }
...
llama_context * llama_init_from_model(
    ...
    if (model->split_mode() == LLAMA_SPLIT_MODE_TENSOR) {
        ...
        if (lm_ggml_is_quantized(params.type_k) || lm_ggml_is_quantized(params.type_v)) {
