...
lm_ggml_cgraph * llama_context::graph_reserve(uint32_t n_tokens, uint32_t n_seqs, uint32_t n_outputs, const llama_memory_context_i * mctx, bool split_only) {
...
std::map<lm_ggml_backend_buffer_type_t, llama_memory_breakdown_data> llama_context::memory_breakdown() const {
    ...
    for (const auto & backend_ptr : backends) {
        ...
        ret[lm_ggml_backend_sched_get_buffer_type(sched.get(), backend)].compute += lm_ggml_backend_sched_get_buffer_size(sched.get(), backend);
    }
    return ret;
}
