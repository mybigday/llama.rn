...
llama_context::~llama_context() {
    ...
    // when training, lm_ggml_opt allocates extra buffers through the scheduler, so the sizes no longer match the expectation
...
static void copy_tensor_async_rows(
    const std::vector<lm_ggml_tensor *> & tensors,
    ...
    for (size_t i = 0; i < tensors.size(); ++i) {
        ...
        const size_t n_elements = lm_ggml_nelements(tensor);
        LM_GGML_ASSERT(lm_ggml_is_contiguous(tensor) && "sampling tensor must be contiguous for async copy");
        LM_GGML_ASSERT(n_elements <= stride);
        LM_GGML_ASSERT((size_t) row * stride + n_elements <= dst.size);
    ...
    ~llama_io_read_device() {
        ...
        for (auto & [buft, mbuf] : mbufs_new) {
            ...
            if (mbuf_cur.n_tensors == mbuf.n_tensors) {
                ...
                for (size_t i = 0; i < mbuf_cur.org.size(); ++i) {
                    if (lm_ggml_nbytes(mbuf_cur.cpy[i]) != lm_ggml_nbytes(mbuf.org[i])) {
                        same_chunking = false;
                        break;
                    }
                }
            ...
            lm_ggml_init_params params_scratch = {
                /*.mem_size   =*/ 2*(mbuf_cur.cpy.size() + mbuf.org.size())*lm_ggml_tensor_overhead(),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
            lm_ggml_context * ctx_scratch = lm_ggml_init(params_scratch);
            ...
            while (src_pos < total) {
                ...
                const size_t src_size = lm_ggml_nbytes(src_t);
                const size_t dst_size = lm_ggml_nbytes(dst_t);
                ...
                const size_t   el   = lm_ggml_element_size(src_t);
                const int64_t n_el = (int64_t) (n_copy / el);

                auto * src_v = lm_ggml_view_1d(ctx_scratch, src_t, n_el, src_off);
                lm_ggml_backend_view_init(src_v);
                auto * dst_v = lm_ggml_view_1d(ctx_scratch, dst_t, n_el, dst_off);
                lm_ggml_backend_view_init(dst_v);

                lm_ggml_backend_tensor_copy(src_v, dst_v);
            ...
            LM_GGML_ASSERT(src_pos == total && dst_pos == total);
            // any tensors left unvisited hold no data
            for (size_t i = src_j; i < mbuf_cur.cpy.size(); ++i) {
                LM_GGML_ASSERT(lm_ggml_nbytes(mbuf_cur.cpy[i]) == 0);
            }
            for (size_t i = dst_i; i < mbuf.org.size(); ++i) {
                LM_GGML_ASSERT(lm_ggml_nbytes(mbuf.org[i]) == 0);
            }

            lm_ggml_free(ctx_scratch);
        }
...
llama_context * llama_init_from_model(
    ...
    if ((model->hparams.is_mla() || model->arch == LLM_ARCH_DEEPSEEK4) && params.type_k != params.type_v) {
        LLAMA_LOG_ERROR("%s: model does not support different K (%s) and V (%s) cache types\n", __func__, lm_ggml_type_name(params.type_k), lm_ggml_type_name(params.type_v));
        return nullptr;
    }

    if (lm_ggml_is_quantized(params.type_v) && params.flash_attn_type != LLAMA_FLASH_ATTN_TYPE_ENABLED) {
        if (params.flash_attn_type == LLAMA_FLASH_ATTN_TYPE_AUTO) {
