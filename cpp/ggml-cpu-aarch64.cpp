...
static void quantize_q8_K_4x8(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t k) {
    assert(QK_K == 256);
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
...
static void quantize_mat_q8_K(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row, int64_t blck_size_interleave) {
    assert(nrow == 4);
    UNUSED(nrow);
...
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, lm_ggml_type PARAM_TYPE> class tensor_traits : public tensor_traits_base {
    ...
    void forward_mul_mat(lm_ggml_compute_params * params, lm_ggml_tensor * op) {
        ...
        if(PARAM_TYPE == LM_GGML_TYPE_Q8_K) {
...
// instance for IQ4
static const tensor_traits<block_iq4_nl, 4, 4, LM_GGML_TYPE_IQ4_NL> iq4_nl_4x4_q8_0;
