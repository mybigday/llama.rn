...
void llama_model_dflash::load_arch_tensors(llama_model_loader &) {
    ...
    const struct lm_ggml_tensor * selector_meta = ml->get_tensor_meta("selector_hidden.weight");
    if (selector_meta) {
...
lm_ggml_tensor * llama_model_dflash::graph<true>::build_inp_embd_enc() const {
    ...
    inp_target->embd = lm_ggml_new_tensor_2d(ctx0, LM_GGML_TYPE_F32, n_embd_inp, n_tokens);
...
static void build_dspark_markov_head(llm_graph_context & g, const llama_model & model, lm_ggml_tensor * tokens) {
    ...
    LM_GGML_ASSERT(w1 && w2 && "DSpark markov weights not loaded");
    ...
    for (int64_t i = i_draft_beg; i < block_drafts; ++i) {
        ...
        lm_ggml_tensor * bias    = g.build_lora_mm(w2, w1_prev, model.dspark_markov_w2_s); // [n_vocab_draft, n_blocks]
        if (model.d2t) {
...
static lm_ggml_tensor * build_dflash2_conv(
        llm_graph_context & g,
        lm_ggml_tensor * hidden,
        lm_ggml_tensor * dynamic,
        lm_ggml_tensor * base,
        int side) {
    ...
    LM_GGML_ASSERT(n_blocks > 0 && n_tokens % n_blocks == 0);
    LM_GGML_ASSERT(dynamic && base && side >= 0 && side < 2);

    const int64_t block_size = n_tokens / n_blocks;
    lm_ggml_context * ctx0 = g.ctx0;
    // lm_ggml_cont copies even when the tensor is already contiguous
    if (!lm_ggml_is_contiguous(hidden) || hidden->ne[1] != n_tokens) {
        hidden = lm_ggml_cont_2d(ctx0, hidden, hidden_size, n_tokens);
    }
    if (!lm_ggml_is_contiguous(dynamic) || dynamic->ne[1] != n_tokens) {
        dynamic = lm_ggml_cont_2d(ctx0, dynamic, dynamic->ne[0], n_tokens);
    }
    lm_ggml_tensor * blocks = lm_ggml_reshape_3d(ctx0, hidden, hidden_size, block_size, n_blocks);
    lm_ggml_tensor * coeffs = lm_ggml_reshape_4d(ctx0, dynamic, n_groups, kernel_size, 2, n_tokens);
    lm_ggml_tensor * coeffs_side = lm_ggml_view_3d(ctx0, coeffs, n_groups, kernel_size, n_tokens,
            coeffs->nb[1], coeffs->nb[3], side * coeffs->nb[2]);

    lm_ggml_tensor * coeff_all = lm_ggml_cont(ctx0, coeffs_side);
    coeff_all = lm_ggml_reshape_4d(ctx0, coeff_all, 1, n_groups, kernel_size, n_tokens);
    coeff_all = lm_ggml_repeat_4d(ctx0, coeff_all, group_size, n_groups, kernel_size, n_tokens);

    lm_ggml_tensor * base_side = lm_ggml_reshape_4d(ctx0,
            lm_ggml_view_1d(ctx0, base, hidden_size * kernel_size, side * base->nb[2]),
            group_size, n_groups, kernel_size, 1);

    lm_ggml_tensor * weight_all = lm_ggml_add(ctx0, coeff_all, base_side);

    lm_ggml_tensor * result = nullptr;
    for (int64_t tap = 0; tap < kernel_size; ++tap) {
        lm_ggml_tensor * values = blocks;
        if (tap > 0) {
            lm_ggml_tensor * zeros = lm_ggml_fill(ctx0,
                    lm_ggml_new_tensor_3d(ctx0, hidden->type, hidden_size, std::min(tap, block_size), n_blocks), 0.0f);
            if (tap < block_size) {
                lm_ggml_tensor * previous = lm_ggml_view_3d(ctx0, blocks, hidden_size, block_size - tap, n_blocks,
                        blocks->nb[1], blocks->nb[2], 0);
                values = lm_ggml_concat(ctx0, zeros, previous, 1);
            } else {
                values = zeros;
            }
        }
        values = lm_ggml_reshape_2d(ctx0, values, hidden_size, n_tokens);

        lm_ggml_tensor * weight = lm_ggml_reshape_2d(ctx0,
                lm_ggml_cont(ctx0, lm_ggml_view_4d(ctx0, weight_all, group_size, n_groups, 1, n_tokens,
                        weight_all->nb[1], weight_all->nb[2], weight_all->nb[3], tap * weight_all->nb[2])),
                hidden_size, n_tokens);

        lm_ggml_tensor * term = lm_ggml_mul(ctx0, weight, values);
        result = result ? lm_ggml_add(ctx0, result, term) : term;
    }
    return result;
}
...
static void build_dflash2_selector(llm_graph_context & g, const llama_model & model, lm_ggml_tensor * tokens) {
    lm_ggml_context * ctx0 = g.ctx0;
    auto         & res  = g.res;
    ...
    const int64_t n_blocks = g.ubatch.n_seqs_unq;
    LM_GGML_ASSERT(n_blocks > 0 && n_tokens % n_blocks == 0);
    LM_GGML_ASSERT(res->t_logits->ne[1] == n_tokens);
    if (!tokens) {
        return;
    }
    ...
    lm_ggml_tensor * candidates  = lm_ggml_top_k(ctx0, res->t_logits, top_k);
    lm_ggml_tensor * logits_rows = lm_ggml_reshape_3d(ctx0, res->t_logits, 1, res->t_logits->ne[0], n_tokens);
    lm_ggml_tensor * unary       = lm_ggml_reshape_2d(ctx0,
            lm_ggml_get_rows(ctx0, logits_rows, candidates), top_k, n_tokens);
    lm_ggml_tensor * gate        = g.build_lora_mm(model.dflash_selector_hidden, res->t_embd);

    // Everything below indexes [.., tokens_per_block, n_blocks]: the block
    // position varies fastest, sequences are the outer dimension.
    lm_ggml_tensor * cand_blk  = lm_ggml_reshape_3d(ctx0, candidates, top_k, tokens_per_block, n_blocks);
    lm_ggml_tensor * unary_blk = lm_ggml_reshape_3d(ctx0, unary,      top_k, tokens_per_block, n_blocks);
    lm_ggml_tensor * gate_blk  = lm_ggml_reshape_3d(ctx0, gate,       rank,  tokens_per_block, n_blocks);
    ...
    auto score_run = [&](int64_t beg_pos, int64_t n_pos, lm_ggml_tensor * pred_ids) {
        lm_ggml_tensor * cand_run = lm_ggml_cont(ctx0, lm_ggml_view_3d(ctx0, cand_blk, top_k, n_pos, n_blocks,
                    cand_blk->nb[1], cand_blk->nb[2], beg_pos * cand_blk->nb[1]));
        lm_ggml_tensor * unary_run = lm_ggml_cont(ctx0, lm_ggml_view_3d(ctx0, unary_blk, top_k, n_pos, n_blocks,
                    unary_blk->nb[1], unary_blk->nb[2], beg_pos * unary_blk->nb[1]));
        lm_ggml_tensor * gate_run = lm_ggml_cont(ctx0, lm_ggml_view_3d(ctx0, gate_blk, rank, n_pos, n_blocks,
                    gate_blk->nb[1], gate_blk->nb[2], beg_pos * gate_blk->nb[1]));

        const int64_t n_pred = pred_ids->ne[0] / (n_pos * n_blocks);

        lm_ggml_tensor * successor = lm_ggml_reshape_4d(ctx0,
                lm_ggml_get_rows(ctx0, model.dflash_selector_next, lm_ggml_reshape_1d(ctx0, cand_run, top_k * n_pos * n_blocks)),
                rank, top_k, n_pos, n_blocks);
        lm_ggml_tensor * predecessor = lm_ggml_reshape_4d(ctx0,
                lm_ggml_get_rows(ctx0, model.dflash_selector_prev, pred_ids),
                rank, n_pred, n_pos, n_blocks);

        lm_ggml_tensor * gate_bcast = lm_ggml_reshape_4d(ctx0, gate_run, rank, 1, n_pos, n_blocks);
        lm_ggml_tensor * cond  = lm_ggml_mul(ctx0, predecessor, lm_ggml_repeat(ctx0, gate_bcast, predecessor));
        lm_ggml_tensor * score = lm_ggml_mul_mat(ctx0, successor, cond);
        if (n_pred == 1) {
            score = lm_ggml_repeat_4d(ctx0, score, top_k, top_k, n_pos, n_blocks);
        }
        lm_ggml_tensor * unary_bcast = lm_ggml_reshape_4d(ctx0, unary_run, top_k, 1, n_pos, n_blocks);
        score = lm_ggml_add(ctx0, score, lm_ggml_repeat(ctx0, unary_bcast, score));

        lm_ggml_tensor * row = lm_ggml_concat(ctx0,
                lm_ggml_cast(ctx0, cand_run, LM_GGML_TYPE_F32),
                lm_ggml_reshape_3d(ctx0, score, top_k * top_k, n_pos, n_blocks), 0);
        return lm_ggml_pad(ctx0, row, n_embd - row_used, 0, 0, 0);
    };

    lm_ggml_tensor * packed = lm_ggml_fill(ctx0,
            lm_ggml_new_tensor_3d(ctx0, LM_GGML_TYPE_F32, n_embd, 1, n_blocks), 0.0f);

    if (block_size > 1) {
        // Position 1 alone: its predecessor is the anchor token, one id per
        // sequence rather than a candidate set.
        lm_ggml_tensor * anchor_ids = lm_ggml_cont_1d(ctx0,
                lm_ggml_view_2d(ctx0, tokens, 1, n_blocks, tokens_per_block * tokens->nb[0], 0), n_blocks);
        packed = lm_ggml_concat(ctx0, packed, score_run(1, 1, anchor_ids), 1);
    }
    if (block_size > 2) {
        lm_ggml_tensor * prev_ids = lm_ggml_reshape_1d(ctx0,
                lm_ggml_cont(ctx0, lm_ggml_view_3d(ctx0, cand_blk, top_k, block_size - 2, n_blocks,
                        cand_blk->nb[1], cand_blk->nb[2], cand_blk->nb[1])),
                top_k * (block_size - 2) * n_blocks);
        packed = lm_ggml_concat(ctx0, packed, score_run(2, block_size - 2, prev_ids), 1);
    }

    packed = lm_ggml_reshape_2d(ctx0, packed, n_embd, block_size * n_blocks);
    g.cb(packed, "dflash2_lattice", -1);
    res->t_h_nextn = packed;
    lm_ggml_build_forward_expand(g.gf, packed);
}
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    auto build_rope = [&](lm_ggml_tensor * cur, lm_ggml_tensor * pos) {
        return rope_type == LM_GGML_ROPE_TYPE_MROPE
            ? lm_ggml_rope_multi(ctx0, cur, pos, nullptr,
                    n_rot, sections, rope_type, n_ctx_orig, freq_base, freq_scale,
                    ext_factor, attn_factor, beta_fast, beta_slow)
            : lm_ggml_rope_ext(ctx0, cur, pos, nullptr,
    ...
    if (ubatch.embd) {
        ...
        inp->embd = lm_ggml_new_tensor_2d(ctx0, LM_GGML_TYPE_F32, n_embd_inp, n_tokens);
        lm_ggml_set_input(inp->embd);

        lm_ggml_tensor * inp_target = inp->embd;
        cb(inp_target, "inp_target_features", -1);

        res->add_input(std::move(inp));

        // fuse the target features through the encoder
        lm_ggml_tensor * inp_g = build_lora_mm(model.fc, inp_target, model.fc_s);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            lm_ggml_tensor * Kcur = build_lora_mm(layer.wk, inp_g, layer.wk_s);
            lm_ggml_tensor * Vcur = build_lora_mm(layer.wv, inp_g, layer.wv_s);
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        lm_ggml_tensor * attn_dynamic = nullptr;
        ...
        lm_ggml_tensor * Qcur = build_lora_mm(layer.wq, noise_norm, layer.wq_s);
        lm_ggml_tensor * Kcur = build_lora_mm(layer.wk, noise_norm, layer.wk_s);
        lm_ggml_tensor * Vcur = build_lora_mm(layer.wv, noise_norm, layer.wv_s);
        ...
        lm_ggml_tensor * ffn_dynamic = nullptr;
