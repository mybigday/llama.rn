...
llama_model_muse_glimmer::graph::graph(const llama_model & model, const llm_graph_params & params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        cur = lm_ggml_rms_norm(ctx0, cur, post_norm_eps);
        cur = lm_ggml_mul(ctx0, cur, model.layers[il].attn_post_norm);
        ...
        cur = lm_ggml_rms_norm(ctx0, cur, post_norm_eps);
        cur = lm_ggml_mul(ctx0, cur, model.layers[il].ffn_post_norm);
