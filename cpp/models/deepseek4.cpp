...
static lm_ggml_tensor * dsv4_with_zero_dep(lm_ggml_context * ctx, lm_ggml_tensor * t, lm_ggml_tensor * dep) {
    if (dep == nullptr) {
        return t;
    }

    lm_ggml_tensor * zero = lm_ggml_scale(ctx, lm_ggml_sum(ctx, dep), 0.0f);
    return lm_ggml_add(ctx, t, zero);
}
...
static lm_ggml_tensor * dsv4_repeat_streams(lm_ggml_context * ctx, lm_ggml_tensor * t, int64_t n_stream) {
    if (t->ne[3] == n_stream) {
        return t;
    }

    LM_GGML_ASSERT(t->ne[3] == 1);
    return lm_ggml_repeat_4d(ctx, t, t->ne[0], t->ne[1], t->ne[2], n_stream);
}

static lm_ggml_tensor * dsv4_build_kq_zero_bias(
        lm_ggml_context * ctx,
    ...
    lm_ggml_tensor * res = lm_ggml_new_tensor_4d(ctx, LM_GGML_TYPE_F32,
            kq_mask->ne[0], kq_mask->ne[1], n_head, kq_mask->ne[3]);
    return lm_ggml_fill(ctx, res, 0.0f);
}
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_hc_weighted_sum(
        lm_ggml_tensor * x,
    ...
    for (int64_t ih = 0; ih < hc; ++ih) {
        ...
        acc = acc ? lm_ggml_add(ctx0, acc, cur) : cur;
    }
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_hc_post(
    ...
    for (int64_t dst = 0; dst < hc; ++dst) {
        ...
        for (int64_t src = 0; src < hc; ++src) {
            ...
            lm_ggml_tensor * comb_src_dst = lm_ggml_view_2d(ctx0, comb, 1, nt, comb->nb[2], dst*comb->nb[0] + src*comb->nb[1]);
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_overlap_compressed_kv_from_state(
    ...
    lm_ggml_tensor * prev_idxs = dsv4_view_1d(ctx0, state_read_idxs, ratio*n_blocks, 0);
    lm_ggml_tensor * cur_idxs  = dsv4_view_1d(ctx0, state_read_idxs, ratio*n_blocks, ratio*n_blocks);

    lm_ggml_tensor * kv_prev = lm_ggml_get_rows(ctx0, kv_state, prev_idxs);
    kv_prev = lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, kv_prev, n_embd_head, ratio*n_blocks, kv_prev->nb[1], 0));
    kv_prev = lm_ggml_reshape_3d(ctx0, kv_prev, n_embd_head, ratio, n_blocks);
    cb(kv_prev, name, il);

    lm_ggml_tensor * score_prev = lm_ggml_get_rows(ctx0, score_state, prev_idxs);
    score_prev = lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, score_prev, n_embd_head, ratio*n_blocks, score_prev->nb[1], 0));
    score_prev = lm_ggml_reshape_3d(ctx0, score_prev, n_embd_head, ratio, n_blocks);
    cb(score_prev, name, il);

    lm_ggml_tensor * kv_cur = lm_ggml_get_rows(ctx0, kv_state, cur_idxs);
    kv_cur = lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, kv_cur, n_embd_head, ratio*n_blocks, kv_cur->nb[1],
            lm_ggml_row_size(kv_cur->type, n_embd_head)));
    kv_cur = lm_ggml_reshape_3d(ctx0, kv_cur, n_embd_head, ratio, n_blocks);

    lm_ggml_tensor * score_cur = lm_ggml_get_rows(ctx0, score_state, cur_idxs);
    score_cur = lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, score_cur, n_embd_head, ratio*n_blocks, score_cur->nb[1],
            lm_ggml_row_size(score_cur->type, n_embd_head)));
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_csa_lid_attention(
    ...
    if (use_fattn && csa_mask->type != LM_GGML_TYPE_F16) {
        csa_mask = lm_ggml_cast(ctx0, csa_mask, LM_GGML_TYPE_F16);
    }
    if (raw_mask->type != csa_mask->type) {
        raw_mask = lm_ggml_cast(ctx0, raw_mask, csa_mask->type);
    }

    lm_ggml_tensor * kq_mask = lm_ggml_concat(ctx0, raw_mask, csa_mask, 0);
    cb(kq_mask, "csa_lid_kq_mask", il);

    lm_ggml_tensor * kq_b = dsv4_build_kq_zero_bias(ctx0, cparams, kq_mask, q->ne[1]);
    lm_ggml_tensor * out = build_attn_mha(q, k_all, k_all, kq_b, kq_mask, sinks, nullptr, kq_scale, il);
    if (k_rot) {
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_hca_attention(
    ...
    if (use_fattn && hca_mask->type != LM_GGML_TYPE_F16) {
        hca_mask = lm_ggml_cast(ctx0, hca_mask, LM_GGML_TYPE_F16);
    }
    if (raw_mask->type != hca_mask->type) {
        raw_mask = lm_ggml_cast(ctx0, raw_mask, hca_mask->type);
    }
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_raw_attention(
    ...
    lm_ggml_tensor * kq_b = dsv4_build_kq_zero_bias(ctx0, cparams, kq_mask, q->ne[1]);
    lm_ggml_tensor * out = build_attn_mha(q, k, k, kq_b, kq_mask, sinks, nullptr, kq_scale, il);
    if (k_rot) {
        out = llama_mul_mat_hadamard(ctx0, out, k_rot);
    }
    cb(out, "attn_raw", il);
...
lm_ggml_tensor * llama_model_deepseek4::graph::build_attention(
    ...
    if (ratio == DSV4_CSA_RATIO && inp_dsv4->get_csa().state_pos) {
        ...
        lm_ggml_tensor * csa_source_kv = lm_ggml_concat(ctx0,
                inp_dsv4->mctx->get_csa_state()->get_kv(ctx0, il), csa_state_kv, 1);
        lm_ggml_tensor * csa_source_score = lm_ggml_concat(ctx0,
        ...
        lm_ggml_tensor * lid_source_kv = lm_ggml_concat(ctx0,
                inp_dsv4->mctx->get_lid_state()->get_kv(ctx0, il), lid_state_kv, 1);
        lm_ggml_tensor * lid_source_score = lm_ggml_concat(ctx0,
...
llama_model_deepseek4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    if (inp_out_ids) {
        ...
        flat = lm_ggml_get_rows(ctx0, flat, inp_out_ids);
        inpL = lm_ggml_reshape_3d(ctx0, flat, n_embd, hc, n_outputs);
    }
