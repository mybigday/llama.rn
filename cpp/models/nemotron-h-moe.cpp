...
llama_model_nemotron_h_moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    LM_GGML_ASSERT(hparams.n_layer_nextn == 1 && "NEMOTRON_H_MOE MTP currently supports a single MTP block");
    ...
    LM_GGML_ASSERT(layer.nextn.eh_proj && layer.nextn.enorm && layer.nextn.hnorm);
    ...
    LM_GGML_ASSERT(tok_embd_w != nullptr && "NEMOTRON_H_MOE MTP requires token embeddings");
    ...
    {
        lm_ggml_tensor * router_logits = build_lora_mm(layer.ffn_gate_inp, cur);
    ...
    LM_GGML_ASSERT(layer.nextn.shared_head_norm && "NEMOTRON_H_MOE MTP: missing final head norm");
    ...
    LM_GGML_ASSERT(head_w != nullptr && "NEMOTRON_H_MOE MTP requires an output projection");
