...
llama_model_talkie::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        lm_ggml_tensor * inp_skip = embd_skip;
        ...
        if (il == n_layer - 1 && inp_out_ids) {
            ...
            inp_skip = lm_ggml_get_rows(ctx0, inp_skip, inp_out_ids);
        }
        ...
        lm_ggml_tensor * skip = lm_ggml_mul(ctx0, inp_skip, model.layers[il].out_scale);
        cb(skip, "embd_skip", il);

        cur = lm_ggml_add(ctx0, cur, skip);
