...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_delta_net_base::build_delta_net_chunking(
    ...
    s = lm_ggml_reshape_4d(ctx0, s, S_v, S_v, 1, H_v * n_seqs);
    ...
    for (int64_t chunk = 0; chunk < n_chunks; chunk++) {
        ...
        // [CS, S_v, 1, H_v * n_seqs]
        lm_ggml_tensor * v_t_p = lm_ggml_mul_mat(ctx0, ch_k_cd, s);
        cb(v_t_p, "v_prime", il);
        ...
        // [S_v, CS, 1, H_v * n_seqs]
        lm_ggml_tensor * attn_inter = lm_ggml_mul_mat(ctx0, s, ch_q_g_exp);
        ...
        s = lm_ggml_mul(ctx0, s, ch_g_last_exp_t);
        s = lm_ggml_add(ctx0, s, kgv);
        cb(s, "dnet_add_ch_state", il);
    }
    ...
    s = lm_ggml_reshape_4d(ctx0, s, S_v, S_v, H_v, n_seqs);
...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_delta_net_base::build_delta_net_autoregressive(
    ...
    s = lm_ggml_add(ctx0, s, kd);

    cb(s, "dnet_add_ar_state", il);

    lm_ggml_tensor * s_q = lm_ggml_mul     (ctx0, s, q);
...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_delta_net_base::build_delta_net_fused(
        lm_ggml_tensor * q,
        lm_ggml_tensor * k,
        lm_ggml_tensor * v,
        lm_ggml_tensor * g,
    ...
    // K=1 (final state only): reshape to 3D (S_v*S_v*H_v, 1, n_seqs) for lm_ggml_gated_delta_net.
    lm_ggml_tensor * s_3d = lm_ggml_reshape_3d(ctx0, s, S_v * S_v * H_v, 1, n_seqs);
    lm_ggml_tensor * result = lm_ggml_gated_delta_net(ctx0, q, k, v, g, b, s_3d);
    if (n_tokens == 1) {
    ...
    lm_ggml_tensor * output = lm_ggml_view_4d(ctx0, result,
            ...
            lm_ggml_row_size(result->type, S_v),
            lm_ggml_row_size(result->type, S_v * H_v),
            lm_ggml_row_size(result->type, S_v * H_v * n_tokens), 0);

    lm_ggml_tensor * new_state = lm_ggml_view_4d(ctx0, result,
            S_v, S_v, H_v, n_seqs,
            lm_ggml_row_size(result->type, S_v),
            lm_ggml_row_size(result->type, S_v * S_v),
            lm_ggml_row_size(result->type, S_v * S_v * H_v),
            lm_ggml_row_size(result->type, S_v * H_v * n_tokens * n_seqs));

    return {output, new_state};
}

std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_delta_net_base::build_delta_net(
        lm_ggml_tensor * q,
        lm_ggml_tensor * k,
        lm_ggml_tensor * v,
        lm_ggml_tensor * g,
...
lm_ggml_tensor * llm_build_delta_net_base::build_recurrent_attn(
        llm_graph_input_rs * inp,
    ...
    lm_ggml_tensor * output = lm_ggml_view_4d(ctx0, gdn_out,
        S_v, H_v, n_seq_tokens, n_seqs,
        lm_ggml_row_size(gdn_out->type, S_v),
        lm_ggml_row_size(gdn_out->type, S_v * H_v),
        lm_ggml_row_size(gdn_out->type, S_v * H_v * n_seq_tokens),
        0);
    cb(output, "attn_output", il);

    const size_t row_size = hparams.n_embd_s() * lm_ggml_element_size(ssm_states_all);
    for (int64_t k_i = 0; k_i < K; ++k_i) {
        const uint32_t cache_slot = (uint32_t) (K - 1 - k_i);
        lm_ggml_tensor * src = lm_ggml_view_4d(ctx0, gdn_out,
            S_v, S_v, H_v, n_seqs,
            lm_ggml_row_size(gdn_out->type, S_v),
