...
static lm_ggml_tensor * bailingmoe3_causal_conv1d(
    ...
    lm_ggml_build_forward_expand(gf, lm_ggml_cpy(ctx0, last_conv_x,
            lm_ggml_view_3d(ctx0, conv_states_all, d_conv - 1, d_inner, n_seqs,
                ...
                (cache_head * total_state_size + qkv * conv_state_size) * lm_ggml_element_size(conv_states_all))));
...
llama_model_bailingmoe3::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (hparams.is_recr(il)) {
            ...
            lm_ggml_tensor * out = lm_ggml_cont(ctx0, result.first);
            lm_ggml_build_forward_expand(gf, lm_ggml_cpy(ctx0, result.second,
                    lm_ggml_view_1d(ctx0, states_all, hparams.n_embd_s() * n_seqs,
                        cache_head * hparams.n_embd_s() * lm_ggml_element_size(states_all))));
