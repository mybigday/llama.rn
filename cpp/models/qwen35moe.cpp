...
llm_build_qwen35moe::llm_build_qwen35moe(const llama_model & model, const llm_graph_params & params) :
    ...
    lm_ggml_tensor * causal_mask =
        lm_ggml_tri(ctx0, lm_ggml_fill(ctx0, lm_ggml_new_tensor_2d(ctx0, LM_GGML_TYPE_F32, CHUNK_SIZE, CHUNK_SIZE), 1.0f),
                    LM_GGML_TRI_TYPE_LOWER);

    lm_ggml_tensor * identity = lm_ggml_diag(ctx0, lm_ggml_fill(ctx0, lm_ggml_new_tensor_1d(ctx0, LM_GGML_TYPE_F32, CHUNK_SIZE), 1.0f));
...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_qwen35moe::build_delta_net_chunking(
        lm_ggml_tensor * q,
        lm_ggml_tensor * k,
        lm_ggml_tensor * v,
        lm_ggml_tensor * g,
...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_qwen35moe::build_delta_net_autoregressive(
        lm_ggml_tensor * q,
        lm_ggml_tensor * k,
        lm_ggml_tensor * v,
        lm_ggml_tensor * g,
...
std::pair<lm_ggml_tensor *, lm_ggml_tensor *> llm_build_qwen35moe::build_qkvz(
...
lm_ggml_tensor * llm_build_qwen35moe::build_norm_gated(
        lm_ggml_tensor * input,
...
lm_ggml_tensor * llm_build_qwen35moe ::build_layer_attn(
...
lm_ggml_tensor * llm_build_qwen35moe ::build_layer_attn_linear(
...
lm_ggml_tensor * llm_build_qwen35moe ::build_layer_ffn(lm_ggml_tensor * cur, const int il) {
