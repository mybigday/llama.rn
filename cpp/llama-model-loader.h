...
struct llama_model_loader {
    ...
    struct lazy_read {
        ...
        bool add(const std::string & name, const lm_ggml_tensor * t, const llama_tensor_weight * w);

        bool any() const {
            return !ranges.empty();
        }

        bool has(const lm_ggml_tensor * t) const {
            return tensors.count(lm_ggml_get_name(t)) > 0;
        }
        ...
        static lm_ggml_backend_buffer_type_t buft();
    ...
    struct ctx_key_comparator {
        bool operator()(const ctx_key & lhs, const ctx_key & rhs) const {
            ...
            return strcmp(lm_ggml_backend_buft_name(lhs.buft), lm_ggml_backend_buft_name(rhs.buft)) < 0;
        }
    };

    std::map<ctx_key, lm_ggml_context_ptr, ctx_key_comparator> ctx_map;
