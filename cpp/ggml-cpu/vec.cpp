...
void lm_ggml_vec_dot_f32(int n, float * LM_GGML_RESTRICT s, size_t bs, const float * LM_GGML_RESTRICT x, size_t bx, const float * LM_GGML_RESTRICT y, size_t by, int nrc) {
   ...
   LM_GGML_UNUSED(bs);
    ...
    float sumf = 0.0f;
        ...
        // maximum number of leftover elements will be less that lm_ggml_f32_epr. Apply predicated svmla on available elements only
        if (np2 < n) {
...
void lm_ggml_vec_dot_f16(int n, float * LM_GGML_RESTRICT s, size_t bs, lm_ggml_fp16_t * LM_GGML_RESTRICT x, size_t bx, lm_ggml_fp16_t * LM_GGML_RESTRICT y, size_t by, int nrc) {
    ...
    lm_ggml_float sumf = 0.0;
...
#if defined(LM_GGML_SIMD)
    #if defined(__ARM_FEATURE_SVE)
        const int lm_ggml_f16_epr = svcnth();
        const int lm_ggml_f16_step = 8 * lm_ggml_f16_epr;
        const int np = n - (n % lm_ggml_f16_step);
        const int np2 = n - (n % lm_ggml_f16_epr);
        ...
        for (int i = 0; i < np; i += lm_ggml_f16_step) {
            lm_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, LM_GGML_F16x_VEC_LOAD(x + i + 0 * lm_ggml_f16_epr, 0), LM_GGML_F16x_VEC_LOAD(y + i + 0 * lm_ggml_f16_epr, 0));
            lm_ggml_sve_f16_fma_widened(&sum2_lo, &sum2_hi, LM_GGML_F16x_VEC_LOAD(x + i + 1 * lm_ggml_f16_epr, 1), LM_GGML_F16x_VEC_LOAD(y + i + 1 * lm_ggml_f16_epr, 1));
            lm_ggml_sve_f16_fma_widened(&sum3_lo, &sum3_hi, LM_GGML_F16x_VEC_LOAD(x + i + 2 * lm_ggml_f16_epr, 2), LM_GGML_F16x_VEC_LOAD(y + i + 2 * lm_ggml_f16_epr, 2));
            lm_ggml_sve_f16_fma_widened(&sum4_lo, &sum4_hi, LM_GGML_F16x_VEC_LOAD(x + i + 3 * lm_ggml_f16_epr, 3), LM_GGML_F16x_VEC_LOAD(y + i + 3 * lm_ggml_f16_epr, 3));
            lm_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, LM_GGML_F16x_VEC_LOAD(x + i + 4 * lm_ggml_f16_epr, 4), LM_GGML_F16x_VEC_LOAD(y + i + 4 * lm_ggml_f16_epr, 4));
            lm_ggml_sve_f16_fma_widened(&sum2_lo, &sum2_hi, LM_GGML_F16x_VEC_LOAD(x + i + 5 * lm_ggml_f16_epr, 5), LM_GGML_F16x_VEC_LOAD(y + i + 5 * lm_ggml_f16_epr, 5));
            lm_ggml_sve_f16_fma_widened(&sum3_lo, &sum3_hi, LM_GGML_F16x_VEC_LOAD(x + i + 6 * lm_ggml_f16_epr, 6), LM_GGML_F16x_VEC_LOAD(y + i + 6 * lm_ggml_f16_epr, 6));
            lm_ggml_sve_f16_fma_widened(&sum4_lo, &sum4_hi, LM_GGML_F16x_VEC_LOAD(x + i + 7 * lm_ggml_f16_epr, 7), LM_GGML_F16x_VEC_LOAD(y + i + 7 * lm_ggml_f16_epr, 7));
        }

        for (int i = np; i < np2; i += lm_ggml_f16_epr) {
            lm_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, LM_GGML_F16x_VEC_LOAD(x + i, 0), LM_GGML_F16x_VEC_LOAD(y + i, 0));
        }
        ...
        if (np2 < n) {
            ...
            lm_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, rx, ry);
        }
        ...
        sumf = lm_ggml_sve_sum_f32x2(sum1_lo, sum1_hi);
    #elif defined(__riscv_v_intrinsic)
...
void lm_ggml_vec_silu_f32(const int n, float * y, const float * x) {
    ...
    for (; i < n; i += vlen) {
        ...
        svst1_f32(pg, y + i, lm_ggml_v_silu(pg, svld1_f32(pg, x + i)));
    }
    ...
    for (int vl; i < n; i += vl) {
        ...
        vfloat32m2_t vy = lm_ggml_v_silu_m2(vx, vl);
...
void lm_ggml_vec_swiglu_f32(const int n, float * y, const float * x, const float * g) {
    ...
    for (; i < n; i += vlen) {
        const svbool_t pg = svwhilelt_b32_s32(i, n);
        svst1_f32(pg, y + i, svmul_f32_x(pg, lm_ggml_v_silu(pg, svld1_f32(pg, x + i)), svld1_f32(pg, g + i)));
    }
#elif defined(__ARM_NEON) && defined(__aarch64__)
    for (; i + 3 < n; i += 4) {
    ...
    for (int vl; i < n; i += vl) {
        ...
        vfloat32m2_t vy = __riscv_vfmul_vv_f32m2(lm_ggml_v_silu_m2(vx, vl), vg, vl);
        __riscv_vse32_v_f32m2(&y[i], vy, vl);
    }
#endif
    for (; i < n; ++i) {
...
lm_ggml_float lm_ggml_vec_cvar_f32(const int n, float * y, const float * x, const float mean) {
    int i = 0;
    lm_ggml_float sum = 0;
...
// ref: https://github.com/ggml-org/llama.cpp/pull/15953#pullrequestreview-3310928344
    ...
    for (; i + 15 < n; i += 16) {
        ...
        sum += (lm_ggml_float)_mm512_reduce_add_ps(_mm512_mul_ps(val, val));
    }
#elif defined(__AVX2__) && defined(__FMA__)
    for (; i + 7 < n; i += 8) {
    ...
    for (; i + 3 < n; i += 4) {
        ...
        vec_xst(val, 0, y + i);
        val = vec_mul(val, val);
        sum += (lm_ggml_float)vec_hsum_f32x4(val);
    }
#elif defined(__riscv_v_intrinsic)
    ...
    sum = (lm_ggml_float)__riscv_vfmv_f_s_f64m1_f64(vsum);
#endif
    for (; i < n; ++i) {
        float val = x[i] - mean;
        y[i] = val;
...
lm_ggml_float lm_ggml_vec_soft_max_f32(const int n, float * y, const float * x, float max) {
    ...
    for (; i < n; i += vlen) {
        const svbool_t pg = svwhilelt_b32_s32(i, n);
        svfloat32_t val = lm_ggml_v_expf(pg, svsub_f32_x(pg, svld1_f32(pg, x + i),
                                                svdup_n_f32_x(pg, max)));
        svst1_f32(pg, y + i, val);
        sum += (lm_ggml_float)svaddv_f32(pg, val);
    }
#elif defined(__ARM_NEON) && defined(__aarch64__)
    for (; i + 3 < n; i += 4) {
