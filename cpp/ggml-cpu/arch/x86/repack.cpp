...
void lm_ggml_quantize_mat_q8_K_4x8(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t k) {
    ...
    UNUSED(nb);
    UNUSED(y);
    lm_ggml_quantize_mat_q8_K_4x8_generic(x, vy, k);
#endif
}
...
static void gemv_q4_b32_8x8_q8_0_lut_avx(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc, __m256i signextendlut) {
    static_assert(
        ...
        for (int64_t x = 0; x < nc / 8; x++) {
            ...
            for (int64_t b = 0; b < nb; b++) {
                ...
                if constexpr (
                        std::is_same_v<block_tx8, block_q4_0x8> ||
                        std::is_same_v<block_tx8, block_iq4_nlx8>) {
                    col_scale_f32 = LM_GGML_F32Cx8_REARRANGE_LOAD(b_ptr[b].d, changemask);
                    ...
                    col_scale_f32 = _mm256_set_ps(
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[7]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[3]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[6]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[2]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[5]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[1]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[4]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[0]));
                }
...
template<typename block_tx8>
static void gemm_q4_b32_8x8_q8_0_lut_avx(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc, __m256i signextendlut) {
    static_assert(
            std::is_same_v<block_tx8, block_q4_0x8> ||
            std::is_same_v<block_tx8, block_iq4_nlx8> ||
            std::is_same_v<block_tx8, block_mxfp4x8>,
        ...
        for (int64_t x = 0; x < anc / 8; x += 2) {
            ...
            for (int64_t b = 0; b < nb; b++) {
                ...
                if constexpr (
                        std::is_same_v<block_tx8, block_q4_0x8> ||
                        std::is_same_v<block_tx8, block_iq4_nlx8>) {
                    col_scale_f32 = LM_GGML_F32Cx8x2_LOAD(b_ptr_0[b].d, b_ptr_1[b].d);
                } else if constexpr (std::is_same_v<block_tx8, block_mxfp4x8>) {
                    //TODO: simd-ify
                    col_scale_f32 = _mm512_set_ps(
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[7]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[6]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[5]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[4]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[3]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[2]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[1]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_1[b].e[0]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[7]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[6]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[5]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[4]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[3]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[2]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[1]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr_0[b].e[0]));
                }
        ...
        for (int64_t x = xstart; x < nc / 8; x++) {
            ...
            for (int64_t b = 0; b < nb; b++) {
                ...
                if constexpr (
                        std::is_same_v<block_tx8, block_q4_0x8> ||
                        std::is_same_v<block_tx8, block_iq4_nlx8>) {
                    col_scale_f32 = LM_GGML_F32Cx8_LOAD(b_ptr[b].d);
                } else if constexpr (std::is_same_v<block_tx8, block_mxfp4x8>) {
                    col_scale_f32 = _mm256_set_ps(
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[7]),
                        LM_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[b].e[6]),
...
void lm_ggml_gemv_iq4_nl_8x8_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc) {
    ...
    lm_ggml_gemv_iq4_nl_8x8_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}
...
void lm_ggml_gemv_mxfp4_8x8_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc) {
    ...
    lm_ggml_gemv_mxfp4_8x8_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}
...
void lm_ggml_gemv_q2_K_8x8_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc) {
    ...
    lm_ggml_gemv_q2_K_8x8_q8_K_generic(n, s, bs, vx, vy, nr, nc);
    ...
    for (; y < anr / 4; y += 4) {
        ...
        for (int64_t x = 0; x < anc / 8; x += 2) {
            ...
            for (int64_t b = 0; b < nb; b++) {
                ...
                const __m512 col_dmin_f32 = LM_GGML_F32Cx8x2_LOAD(b_ptr_0[b].dmin, b_ptr_1[b].dmin);
...
void lm_ggml_gemm_mxfp4_8x8_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, const void * LM_GGML_RESTRICT vy, int nr, int nc) {
    ...
    lm_ggml_gemm_mxfp4_8x8_q8_0_generic(n, s, bs, vx, vy, nr, nc);
}
