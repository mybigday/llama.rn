...
template <> void gemv<block_q2_K, 8, 8, LM_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
...
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, lm_ggml_type PARAM_TYPE> class tensor_traits : public tensor_traits_base {
    ...
    void forward_mul_mat_one_chunk(lm_ggml_compute_params * params, lm_ggml_tensor * op, int64_t src0_start, int64_t src0_end) {
    ...
    void forward_mul_mat(lm_ggml_compute_params * params, lm_ggml_tensor * op) {
        ...
        int64_t nr = lm_ggml_nrows(op->src[0]);
