...
static bool lm_ggml_hexagon_supported_mul_mat(const struct lm_ggml_hexagon_session * sess, const struct lm_ggml_tensor * dst) {
    ...
    if (src0->buffer &&
        (!lm_ggml_backend_buffer_is_hexagon(src0->buffer) || lm_ggml_backend_hexagon_buffer_get_sess(src0->buffer) != sess)) {
        return false;
    }
    if (src1->buffer &&
        (!lm_ggml_backend_buffer_is_hexagon(src1->buffer) || lm_ggml_backend_hexagon_buffer_get_sess(src1->buffer) != sess)) {
        return false;
    }
    if (dst->buffer &&
        (!lm_ggml_backend_buffer_is_hexagon(dst->buffer) || lm_ggml_backend_hexagon_buffer_get_sess(dst->buffer) != sess)) {
        return false;
    }
...
static bool lm_ggml_hexagon_supported_mul_mat_id(const struct lm_ggml_hexagon_session * sess, const struct lm_ggml_tensor * op) {
    ...
    if (src2->buffer &&
        (!lm_ggml_backend_buffer_is_hexagon(src2->buffer) || lm_ggml_backend_hexagon_buffer_get_sess(src2->buffer) != sess)) {
        return false;
    }
    if (dst->buffer &&
        (!lm_ggml_backend_buffer_is_hexagon(dst->buffer) || lm_ggml_backend_hexagon_buffer_get_sess(dst->buffer) != sess)) {
...
static void lm_ggml_hexagon_mul_mat(const struct lm_ggml_tensor * op, uint32_t flags) {
    ...
    auto src0_buf = static_cast<lm_ggml_backend_hexagon_buffer_context *>(src0->buffer->context);
    auto src1_buf = static_cast<lm_ggml_backend_hexagon_buffer_context *>(src1->buffer->context);
    auto dst_buf  = static_cast<lm_ggml_backend_hexagon_buffer_context *>(dst->buffer->context);
    ...
    bufs[0].size   = lm_ggml_nbytes(src0);
    bufs[0].flags  = 0;
    ...
    bufs[1].size   = lm_ggml_nbytes(src1);
    ...
    bufs[2].size   = lm_ggml_nbytes(dst);
...
static void lm_ggml_hexagon_mul_mat_id(const struct lm_ggml_tensor * op, uint32_t flags) {
    ...
    auto src0_buf = static_cast<lm_ggml_backend_hexagon_buffer_context *>(src0->buffer->context);
    auto src1_buf = static_cast<lm_ggml_backend_hexagon_buffer_context *>(src1->buffer->context);
    auto src2_buf = static_cast<lm_ggml_backend_hexagon_buffer_context *>(src2->buffer->context);
    auto dst_buf  = static_cast<lm_ggml_backend_hexagon_buffer_context *>(dst->buffer->context);
    ...
    bufs[2].size   = lm_ggml_nbytes(src2);
    ...
    bufs[3].size   = lm_ggml_nbytes(dst);
...
static void lm_ggml_hexagon_binary(const struct lm_ggml_tensor * op, uint32_t flags) {
    ...
    lm_ggml_hexagon_session * sess = src0_buf->sess;

    if (opt_verbose) {
        char dims[64 * LM_GGML_MAX_SRC];
        char strides[16 * LM_GGML_MAX_SRC];
        char types[16 * LM_GGML_MAX_SRC];
        char buffs[64 * LM_GGML_MAX_SRC];
        char names[64 * LM_GGML_MAX_SRC];
        ...
        HEX_VERBOSE("ggml-hex: %s %s : %s : %s : %s : %s : %s : flags 0x%x\n", sess->name.c_str(),
                    lm_ggml_op_name(node->op), names, dims, types, strides, buffs, req.flags);
        if (opt_verbose > 1) {
            hex_dump_dspbuf(src0, &bufs[0]);
            hex_dump_dspbuf(src1, &bufs[1]);
            hex_dump_dspbuf(dst, &bufs[2]);
...
static void lm_ggml_hexagon_unary(const struct lm_ggml_tensor * op, uint32_t flags) {
    ...
    bufs[n_bufs].size   = lm_ggml_nbytes(src0);
    ...
    if (src1) {
        ...
        bufs[n_bufs].size   = lm_ggml_nbytes(src1);
        bufs[n_bufs].flags  = (DSPQUEUE_BUFFER_FLAG_FLUSH_SENDER |         // Flush CPU
                              DSPQUEUE_BUFFER_FLAG_INVALIDATE_RECIPIENT);  // Invalidate DSP
        ++n_bufs;
    }
    ...
    bufs[n_bufs].size   = lm_ggml_nbytes(dst);
...
static void lm_ggml_hexagon_rope(const struct lm_ggml_tensor * op, uint32_t flags) {
    ...
    if (src2) {
        ...
        bufs[n_bufs].size   = lm_ggml_nbytes(src2);
        bufs[n_bufs].flags  = (DSPQUEUE_BUFFER_FLAG_FLUSH_SENDER |         // Flush CPU
                              DSPQUEUE_BUFFER_FLAG_INVALIDATE_RECIPIENT);  // Invalidate DSP
        ++n_bufs;
    }
