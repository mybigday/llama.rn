...
static void lm_ggml_hexagon_precompute_fused_ffn_params(
    const struct lm_ggml_hexagon_session * sess,
    const struct lm_ggml_tensor * src0,
    const struct lm_ggml_tensor * src1,
    struct htp_mm_kernel_params * kparams
...
static void repack_q4_0_tiled(lm_ggml_tensor * t, const void * data, size_t size) {
...
static void repack_tiled_q4_0(void * data, const lm_ggml_tensor * t, size_t size) {
...
static void repack_q4_1_tiled(lm_ggml_tensor * t, const void * data, size_t size) {
...
static void repack_tiled_q4_1(void * data, const lm_ggml_tensor * t, size_t size) {
...
static void repack_q8_0_tiled(lm_ggml_tensor * t, const void * data, size_t size) {
...
static void repack_tiled_q8_0(void * data, const lm_ggml_tensor * t, size_t size) {
...
static void repack_mxfp4_tiled(lm_ggml_tensor * t, const void * data, size_t size) {
...
static void repack_tiled_mxfp4(void * data, const lm_ggml_tensor * t, size_t size) {
...
static size_t lm_ggml_backend_hexagon_buffer_type_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * t) {
    if (t->type == LM_GGML_TYPE_Q4_0 || t->type == LM_GGML_TYPE_Q4_1 || t->type == LM_GGML_TYPE_Q8_0 || t->type == LM_GGML_TYPE_IQ4_NL || t->type == LM_GGML_TYPE_MXFP4) {
...
static size_t lm_ggml_backend_hexagon_buffer_type_get_max_size(lm_ggml_backend_buffer_type_t buffer_type) {
    auto * context = static_cast<lm_ggml_backend_hexagon_buffer_type_context *>(buffer_type->context);
...
struct lm_ggml_hexagon_opbatch {
    ...
    bool same_shape(const htp_tensor * h, const lm_ggml_tensor * t) const {
        int64_t ne0 = t->ne[0];
        int64_t ne1 = t->ne[1];
        const bool is_repack = lm_ggml_backend_buffer_is_hexagon_repack(t->buffer) && lm_ggml_hexagon_is_repack_type(t->type);
        if (is_repack) {
...
static bool lm_ggml_hexagon_flash_attn_is_hmx_eligible(
    ...
    if (k->type != LM_GGML_TYPE_F16 || v->type != LM_GGML_TYPE_F16) {
        return false;
    }
...
static void lm_ggml_hexagon_precompute_matmul_params(
    ...
    lm_ggml_hexagon_precompute_hvx_mm_params(sess, src0, src1, dst, wtype, ne02, ne03, ne10, ne11, ne12, ne13, is_matmul_id, vtcm_budget, kparams);
...
static void lm_ggml_hexagon_precompute_fused_ffn_params(
    const struct lm_ggml_hexagon_session * sess,
    const struct lm_ggml_tensor * src0, // Wgate
    const struct lm_ggml_tensor * src1, // y
    struct htp_mm_kernel_params * kparams
) {
    memset(kparams, 0, sizeof(*kparams));
...
static bool is_qkv_mergeable(const lm_ggml_tensor * n_q, const lm_ggml_tensor * n_k, const lm_ggml_tensor * n_v) {
...
static bool try_fuse_node(const lm_ggml_hexagon_session * sess, const lm_ggml_cgraph * graph, int & i, std::vector<htp_opnode> & nodes) {
    if (!opt_opfusion) {
        return false;
    }
    ...
    if (is_mergeable_mul_mat(n)) {
        lm_ggml_tensor * n1 = (i + 1 < graph->n_nodes) ? graph->nodes[i + 1] : nullptr;
        lm_ggml_tensor * n2 = (i + 2 < graph->n_nodes) ? graph->nodes[i + 2] : nullptr;
        if (is_qkv_mergeable(n, n1, n2)) {
            struct htp_mm_kernel_params kparams;
            lm_ggml_hexagon_precompute_fused_qkv_params(sess, n1->src[0], n1->src[1], &kparams);
            if ((size_t)kparams.vtcm_size <= sess->vtcm_size) {
        ...
        if (is_mergeable_mul_mat_pair(n, n1)) {
            struct htp_mm_kernel_params kparams;
            lm_ggml_hexagon_precompute_fused_ffn_params(sess, n->src[0], n->src[1], &kparams);
            if ((size_t)kparams.vtcm_size <= sess->vtcm_size) {
    ...
    if (n->op == LM_GGML_OP_MUL_MAT && next_node) {
        if (next_node->op == LM_GGML_OP_ADD && op_is_compute(next_node) && lm_ggml_can_fuse(graph, i, { LM_GGML_OP_MUL_MAT, LM_GGML_OP_ADD })) {
            if (next_node->src[0] == n || next_node->src[1] == n) {
                struct htp_mm_kernel_params kparams;
                lm_ggml_hexagon_precompute_matmul_params(sess, n->src[0], n->src[1], next_node, &kparams);
                if ((size_t)kparams.vtcm_size <= sess->vtcm_size) {
