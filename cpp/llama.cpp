...
struct llama_context {
    ...
    ~llama_context() {
        ...
        lm_ggml_backend_buffer_free(buf_input);
        lm_ggml_free(ctx_input);
    }
    ...
    // input tensors
    lm_ggml_backend_buffer_t buf_input = nullptr;
    lm_ggml_context * ctx_input = nullptr;
    ...
    struct lm_ggml_tensor * inp_KQ_mask;   // F32 [n_ctx, n_batch]
    struct lm_ggml_tensor * inp_KQ_pos;    // F32 [n_ctx]
    struct lm_ggml_tensor * inp_K_shift;   // I32 [n_ctx]
...
static bool llama_kv_cache_init(
    ...
    for (int i = 0; i < (int) n_layer; i++) {
        ...
        lm_ggml_tensor * k = lm_ggml_new_tensor_1d(ctx, ktype, n_embd_k_gqa*n_ctx);
        lm_ggml_tensor * v = lm_ggml_new_tensor_1d(ctx, vtype, n_embd_v_gqa*n_ctx);
...
static struct lm_ggml_tensor * llm_build_inp_embd(
        ...
        const llama_hparams & hparams,
         ...
         struct lm_ggml_tensor * tok_embd,
         struct lm_ggml_tensor * inp_tokens,
         struct lm_ggml_tensor * inp_embd,
    ...
    if (batch.token) {
        struct lm_ggml_tensor * inp_tokens_v = lm_ggml_view_1d(ctx, inp_tokens, batch.n_tokens, 0);
        cb(inp_tokens, "inp_tokens", -1);

        inpL = lm_ggml_get_rows(ctx, tok_embd, inp_tokens_v);
    } else {
#ifdef LM_GGML_USE_MPI
        LM_GGML_ASSERT(false && "not implemented");
#endif

        inpL = lm_ggml_view_2d(ctx, inp_embd, n_embd, batch.n_tokens, inp_embd->nb[1], 0);
    }
...
#if defined(LM_GGML_USE_VULKAN) || defined(LM_GGML_USE_KOMPUTE) || defined(LM_GGML_USE_SYCL)
#pragma message("TODO: ALiBi support in lm_ggml_soft_max_ext is not implemented for Vulkan, Kompute, and SYCL")
...
struct llm_build_context {
    ...
    struct lm_ggml_cgraph * build_llama() {
        ...
        struct lm_ggml_tensor * inp_pos = lm_ggml_view_1d(ctx0, lctx.inp_pos, n_tokens, 0);
        cb(inp_pos, "inp_pos", -1);

        // KQ_mask (mask for 1 head, it will be broadcasted to all heads)
        struct lm_ggml_tensor * KQ_mask = lm_ggml_view_2d(ctx0, lctx.inp_KQ_mask, n_kv, n_tokens, n_kv*lm_ggml_type_size(lctx.inp_KQ_mask->type), 0);
    ...
    struct lm_ggml_cgraph * build_baichuan() {
        ...
        struct lm_ggml_tensor * KQ_pos = lm_ggml_view_1d(ctx0, lctx.inp_KQ_pos, n_kv, 0);
        cb(KQ_pos, "KQ_pos", -1);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self attention
            {
                ...
                struct lm_ggml_tensor * qpass = lm_ggml_view_3d(
                        ...
                        lm_ggml_element_size(tmpq) * n_embd_head,
                        lm_ggml_element_size(tmpq) * n_embd_head * n_head,
                        lm_ggml_element_size(tmpq) * hparams.n_rot
                        );
                cb(qpass, "qpass", il);
                ...
                struct lm_ggml_tensor * kpass = lm_ggml_view_3d(
                        ...
                        lm_ggml_element_size(tmpk) * n_embd_head,
                        lm_ggml_element_size(tmpk) * n_embd_head * n_head,
                        lm_ggml_element_size(tmpk) * hparams.n_rot
                        );
                cb(kpass, "kpass", il);
    ...
    struct lm_ggml_cgraph * build_bert() {
        ...
        const size_t stride1 = n_tokens * lm_ggml_type_size(lctx.inp_tokens->type);
        struct lm_ggml_tensor * inp_pos = lm_ggml_view_1d(ctx0, lctx.inp_pos, n_tokens, 0);
        struct lm_ggml_tensor * inp_mean = lm_ggml_view_2d(ctx0, lctx.inp_mean, n_tokens, n_tokens, stride1, 0);
        struct lm_ggml_tensor * inp_cls = lm_ggml_view_1d(ctx0, lctx.inp_cls, n_tokens, 0);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            if (model.arch == LLM_ARCH_BERT) {
                struct lm_ggml_tensor * Qcur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].wq, cur), model.layers[il].bq);
                cb(Qcur, "Qcur", il);

                struct lm_ggml_tensor * Kcur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].wk, cur), model.layers[il].bk);
                cb(Kcur, "Kcur", il);

                struct lm_ggml_tensor * Vcur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].wv, cur), model.layers[il].bv);
                cb(Vcur, "Vcur", il);
        ...
        if (pooling_type == LLAMA_POOLING_MEAN) {
            ...
            LM_GGML_ASSERT(pooling_type == LLAMA_POOLING_NONE && "Invalid pooling type");
        }
    ...
    struct lm_ggml_cgraph * build_plamo() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                Qcur = lm_ggml_rope_custom(
                        ctx0, lm_ggml_reshape_3d(ctx0, Qcur, hparams.n_rot, n_head,    n_tokens), inp_pos,
                        n_embd_head, 2, 0, n_orig_ctx, freq_base, freq_scale,
                        ext_factor, attn_factor, beta_fast, beta_slow);
                cb(Qcur, "Qcur", il);

                Kcur = lm_ggml_rope_custom(
                        ctx0, lm_ggml_reshape_3d(ctx0, Kcur, hparams.n_rot, n_head_kv, n_tokens), inp_pos,
                        n_embd_head, 2, 0, n_orig_ctx, freq_base, freq_scale,
                        ext_factor, attn_factor, beta_fast, beta_slow);
                cb(Kcur, "Kcur", il);
    ...
    llm_build_cb cb = [&](struct lm_ggml_tensor * cur, const char * name, int il) {
        ...
        if (!lctx.cparams.offload_kqv) {
            if (strcmp(name, "kqv_merged_cont") == 0) {
                ...
                lm_ggml_backend_sched_set_node_backend(lctx.sched, cur, lctx.backend_cpu);
            }
        }
    };
...
static void llama_set_inputs(llama_context & lctx, const llama_batch & batch) {
...

    {
        ...
        assert(lm_ggml_backend_buffer_is_host(lctx.inp_KQ_mask->buffer));
    ...
    if (hparams.need_kq_pos) {
        ...
        assert(lm_ggml_backend_buffer_is_host(lctx.inp_KQ_pos->buffer));
...
static int llama_decode_internal(
    ...
    lm_ggml_backend_sched_graph_compute(lctx.sched, gf);
    ...
    if (res) {
        ...
        lm_ggml_backend_t res_backend = lm_ggml_backend_sched_get_node_backend(lctx.sched, res);
        LM_GGML_ASSERT(res_backend != nullptr);
        if (batch.logits) {
            ...
            for (uint32_t i = 0; i < n_tokens; i++) {
                ...
                lm_ggml_backend_tensor_get_async(res_backend, res, logits_out.data() + (n_vocab*i), (n_vocab*i)*sizeof(float), n_vocab*sizeof(float));
#ifndef NDEBUG
                logits_valid[i] = true;
#endif
            }
            ...
            logits_out.resize(n_vocab * n_tokens);
            lm_ggml_backend_tensor_get_async(res_backend, res, logits_out.data(), 0, n_vocab*n_tokens*sizeof(float));
#ifndef NDEBUG
            std::fill(logits_valid.begin(), logits_valid.end(), true);
#endif
        } else {
            logits_out.resize(n_vocab);
            lm_ggml_backend_tensor_get_async(res_backend, res, logits_out.data(), (n_vocab*(n_tokens - 1))*sizeof(float), n_vocab*sizeof(float));
#ifndef NDEBUG
            logits_valid[0] = true;
#endif
        }
        lm_ggml_backend_synchronize(res_backend);
    }
    ...
    if (!lctx.embedding.empty()) {
        ...
        lm_ggml_backend_t embeddings_backend = lm_ggml_backend_sched_get_node_backend(lctx.sched, embeddings);
        lm_ggml_backend_tensor_get_async(embeddings_backend, embeddings, embedding_out.data(), embd_pos*sizeof(float), embd_size*sizeof(float));
        lm_ggml_backend_synchronize(embeddings_backend);
    }
...
void llama_sample_classifier_free_guidance(
    ...
    LM_GGML_ASSERT(n_vocab == candidates->size);
...
static lm_ggml_type get_k_quant_type(quantize_state_internal & qs, lm_ggml_type new_type, const lm_ggml_tensor * tensor, llama_ftype ftype) {
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
        ...
        if (name.find("attn_v.weight") != std::string::npos) {
            ...
            else new_type = LM_GGML_TYPE_Q2_K;
            ++qs.i_attention_wv;
        }
        else if (name.find("ffn_down") != std::string::npos) {
            if (qs.i_ffn_down < qs.n_ffn_down/8) new_type = LM_GGML_TYPE_Q2_K;
            ++qs.i_ffn_down;
        }
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) {
            ...
            new_type = qs.model.hparams.n_gqa() >= 4 ? LM_GGML_TYPE_Q4_K : !qs.has_imatrix ? LM_GGML_TYPE_Q3_K : LM_GGML_TYPE_IQ3_XXS;
        }
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) new_type = LM_GGML_TYPE_Q3_K;
            ...
            if (i_layer < n_layer/8) new_type = LM_GGML_TYPE_Q5_K;
        }
        ...
        if (arch != LLM_ARCH_FALCON) {
            if (qs.model.hparams.n_expert == 8) {
                ...
                if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K  ) new_type = LM_GGML_TYPE_Q3_K;
                else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_XXS) new_type = LM_GGML_TYPE_Q3_K;
    ...
    if (new_type == LM_GGML_TYPE_Q2_K || new_type == LM_GGML_TYPE_Q3_K || new_type == LM_GGML_TYPE_Q4_K ||
        ...
        new_type == LM_GGML_TYPE_IQ2_XS || new_type == LM_GGML_TYPE_IQ2_XXS ||
        new_type == LM_GGML_TYPE_IQ3_XXS || ftype == LLAMA_FTYPE_MOSTLY_IQ1_S) {
        int nx = tensor->ne[0];
        int ny = tensor->ne[1];
        if (nx % QK_K != 0) {
    ...
    if (convert_incompatible_tensor) {
        switch (new_type) {
            ...
            case LM_GGML_TYPE_Q3_K: new_type = LM_GGML_TYPE_IQ4_NL; break;
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    switch (params->ftype) {
        ...
        case LLAMA_FTYPE_MOSTLY_IQ2_XXS: quantized_type = LM_GGML_TYPE_IQ2_XXS; break;
        case LLAMA_FTYPE_MOSTLY_IQ2_XS:  quantized_type = LM_GGML_TYPE_IQ2_XS;  break;
        case LLAMA_FTYPE_MOSTLY_IQ3_XXS: quantized_type = LM_GGML_TYPE_IQ3_XXS; break;
        case LLAMA_FTYPE_MOSTLY_IQ1_S:   quantized_type = LM_GGML_TYPE_IQ1_S;   break;
        case LLAMA_FTYPE_MOSTLY_IQ4_NL:  quantized_type = LM_GGML_TYPE_IQ4_NL;  break;
    ...
    for (int i = 0; i < ml.n_tensors; ++i) {
        ...
        if (!quantize) {
            ...
            if (nthread_use < 2) {
                new_size = lm_ggml_quantize_chunk(new_type, f32_data, new_data, 0, nrows, n_per_row, hist_cur.data(), imatrix);
            } else {
                int counter = 0;
                new_size = 0;
                auto compute = [&mutex, &counter, &hist_cur, &new_size, new_type, f32_data, new_data, chunk_size,
                    ...
                    while (true) {
                        ...
                        local_size += lm_ggml_quantize_chunk(new_type, f32_data, new_data,
            ...
            LLAMA_LOG_INFO("size = %8.2f MiB -> %8.2f MiB", lm_ggml_nbytes(tensor)/1024.0/1024.0, new_size/1024.0/1024.0);
            int64_t tot_count = 0;
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        // graph inputs
        {
            lm_ggml_init_params init_params = {
                /* .mem_size   */ lm_ggml_tensor_overhead()*8,
                /* .mem_buffer */ nullptr,
                /* .no_alloc   */ true,
            };
            ctx->ctx_input = lm_ggml_init(init_params);

            ctx->inp_tokens  = lm_ggml_new_tensor_1d(ctx->ctx_input, LM_GGML_TYPE_I32, cparams.n_batch);
            ctx->inp_embd    = lm_ggml_new_tensor_2d(ctx->ctx_input, LM_GGML_TYPE_F32, hparams.n_embd, cparams.n_batch);
            ctx->inp_pos     = lm_ggml_new_tensor_1d(ctx->ctx_input, LM_GGML_TYPE_I32, cparams.n_batch);
            ctx->inp_KQ_mask = lm_ggml_new_tensor_2d(ctx->ctx_input, LM_GGML_TYPE_F32, cparams.n_ctx, cparams.n_batch);
            ctx->inp_KQ_pos  = lm_ggml_new_tensor_1d(ctx->ctx_input, LM_GGML_TYPE_F32, cparams.n_ctx);
            ctx->inp_K_shift = lm_ggml_new_tensor_1d(ctx->ctx_input, LM_GGML_TYPE_I32, cparams.n_ctx);
            ctx->inp_mean    = lm_ggml_new_tensor_2d(ctx->ctx_input, LM_GGML_TYPE_F32, cparams.n_batch, cparams.n_batch);
            ctx->inp_cls     = lm_ggml_new_tensor_1d(ctx->ctx_input, LM_GGML_TYPE_I32, cparams.n_batch);

            lm_ggml_set_name(ctx->inp_tokens,  "inp_tokens");
            lm_ggml_set_name(ctx->inp_embd,    "inp_embd");
            lm_ggml_set_name(ctx->inp_pos,     "inp_pos");
            lm_ggml_set_name(ctx->inp_KQ_mask, "inp_KQ_mask");
            lm_ggml_set_name(ctx->inp_KQ_pos,  "inp_KQ_pos");
            lm_ggml_set_name(ctx->inp_K_shift, "inp_K_shift");
            lm_ggml_set_name(ctx->inp_mean,    "inp_mean");
            lm_ggml_set_name(ctx->inp_cls,     "inp_cls");

            ctx->buf_input = lm_ggml_backend_alloc_ctx_tensors_from_buft(ctx->ctx_input, llama_default_buffer_type_cpu(true));

            LLAMA_LOG_INFO("%s: %10s input buffer size   = %8.2f MiB\n", __func__,
                    lm_ggml_backend_buffer_name(ctx->buf_input),
                    lm_ggml_backend_buffer_get_size(ctx->buf_input) / 1024.0 / 1024.0);
        }
        ...
        // scheduler and compute buffers
        {
            ...
            ctx->buf_compute_meta.resize(lm_ggml_tensor_overhead()*LLAMA_MAX_NODES + lm_ggml_graph_overhead());

            ctx->sched = lm_ggml_backend_sched_new(ctx->backends.data(), backend_buft.data(), ctx->backends.size(), LLAMA_MAX_NODES);
    ...
    // copy kv cache
    {
        ...
        if (kv_buf_size) {
            ...
            for (int il = 0; il < (int) n_layer; ++il) {
                size_t k_size = lm_ggml_row_size(kv_self.k_l[il]->type, n_embd_k_gqa*kv_head);
                tmp_buf.resize(k_size);
                lm_ggml_backend_tensor_get(kv_self.k_l[il], tmp_buf.data(), 0, tmp_buf.size());
                data_ctx->write(tmp_buf.data(), tmp_buf.size());

                // v is not contiguous, copy row by row
                size_t v_row_size = lm_ggml_row_size(kv_self.v_l[il]->type, kv_head);
                size_t v_row_stride = lm_ggml_row_size(kv_self.v_l[il]->type, n_ctx);
...
size_t llama_set_state_data(struct llama_context * ctx, uint8_t * src) {
    ...
    // set logits
    {
        ...
        LM_GGML_ASSERT(ctx->logits.capacity() >= logits_size);
