...
struct llama_context {
    ...
    struct lm_ggml_tensor * inp_KQ_pos;    // F32 [n_kv]
...
struct llama_model_loader {
    ...
    struct lm_ggml_tensor * create_tensor_for(struct lm_ggml_context * ctx, const struct lm_ggml_tensor * cur) {
...
static struct lm_ggml_tensor * llm_build_kqv(
    ...
    if (cparams.flash_attn) {
        ...
        LM_GGML_ASSERT(kq_pos == nullptr && "ALiBi is not yet supported with Flash Attention");

        // split cached v into n_head heads (not transposed)
        struct lm_ggml_tensor * v =
            lm_ggml_view_3d(ctx, kv.v_l[il],
                    n_embd_head_v, n_kv, n_head_kv,
                    lm_ggml_row_size(kv.v_l[il]->type, n_embd_k_gqa),
                    lm_ggml_row_size(kv.v_l[il]->type, n_embd_head_k),
                    0);
        cb(v, "v", il);

        cur = lm_ggml_flash_attn_ext(ctx, q, k, v, kq_mask, kq_scale);

        if (model.arch == LLM_ARCH_PHI2 || model.arch == LLM_ARCH_PHI3) {
            lm_ggml_flash_attn_ext_set_prec(cur, LM_GGML_PREC_F32);
        }

        cur = lm_ggml_reshape_2d(ctx, cur, n_embd_head_k*n_head, n_tokens);
    } else {
...
#pragma message("ref:  https://github.com/ggerganov/llama.cpp/pull/5488")
        ...
        cur = lm_ggml_cont_2d(ctx, kqv_merged, n_embd_head_k*n_head, n_tokens);
...
struct llm_build_context {
    ...
    struct lm_ggml_tensor * build_inp_KQ_pos(bool causal = true) {
        if (causal) {
            ...
            lctx.inp_KQ_pos = lm_ggml_new_tensor_1d(ctx0, LM_GGML_TYPE_F32, n_tokens);
        }
        cb(lctx.inp_KQ_pos, "KQ_pos", -1);
        lm_ggml_set_input(lctx.inp_KQ_pos);
        return flash_attn ? lm_ggml_cast(ctx0, lctx.inp_KQ_pos, LM_GGML_TYPE_F16) : lctx.inp_KQ_pos;
    }
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
        ...
        if (hparams.causal_attn) {
            ...
            if (!kv_self.recurrent) {
                ...
                kv_self.n = std::min(kv_self.size, std::max(256u, LM_GGML_PAD(llama_kv_cache_cell_max(kv_self), 256)));
