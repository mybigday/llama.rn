...
llama_memory_recurrent::llama_memory_recurrent(
    ...
    auto ctx_for_buft = [&](lm_ggml_backend_buffer_type_t buft) -> lm_ggml_context * {
        ...
        if (it == ctx_map.end()) {
            lm_ggml_init_params params = {
                ...
                /*.mem_size   =*/ size_t((hparams.ple_conv_state() > 0 ? 3u : 2u)*n_layer*lm_ggml_tensor_overhead()),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    for (int i = 0; i < n_layer; i++) {
        ...
        if (hparams.ple_conv_state() > 0 && hparams.is_ple(i)) {
            lm_ggml_tensor * p = lm_ggml_new_tensor_2d(ctx, type_r, hparams.ple_conv_state(), n_rows);
            lm_ggml_format_name(p, "cache_ple_r_l%d", i);
            p_l[i] = p;
        }
    }
...

    {
        ...
        LLAMA_LOG_INFO("%s: size = %7.2f MiB (%6u cells, %3d layers, %2u seqs %2u rs_seq), R (%s): %7.2f MiB, S (%s): %7.2f MiB, P (%s): %7.2f MiB\n", __func__,
                ...
                lm_ggml_type_name(type_r), (float)memory_size_p / (1024.0f * 1024.0f));
    }
}
...
void llama_memory_recurrent::state_write_data(llama_io_write_i & io, const std::vector<std::pair<uint32_t, uint32_t>> & cell_ranges) const {
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (p_l[il] != nullptr) {
            const uint64_t p_size_row = lm_ggml_row_size(p_l[il]->type, hparams.ple_conv_state());
...
bool llama_memory_recurrent::state_read_data(llama_io_read_i & io, uint32_t cell_count) {
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (p_l[il] != nullptr) {
            ...
            const size_t p_size_row = lm_ggml_row_size(p_l[il]->type, hparams.ple_conv_state());
...
lm_ggml_tensor * llama_memory_recurrent_context::get_p_l(int32_t il) const {
    return mem->p_l[il];
}
