...
llama_dsv4_comp_state::llama_dsv4_comp_state(
    ...
    for (uint32_t il = 0; il < hparams.n_layer(); ++il) {
        ...
        lm_ggml_tensor * kv    = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, n_embd_state, state_size, n_stream);
        lm_ggml_tensor * score = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, n_embd_state, state_size, n_stream);
...
lm_ggml_tensor * llama_dsv4_comp_state::get_kv(lm_ggml_context * ctx, int32_t il) const {
    ...
    return lm_ggml_reshape_2d(ctx, state, state->ne[0], state->ne[1]*state->ne[2]);
}
...
lm_ggml_tensor * llama_dsv4_comp_state::cpy_kv(lm_ggml_context * ctx, lm_ggml_tensor * cur, lm_ggml_tensor * idxs, int32_t il) const {
    return lm_ggml_set_rows(ctx, get_kv(ctx, il), cur, idxs);
}
...
lm_ggml_tensor * llama_dsv4_comp_state::cpy_score(lm_ggml_context * ctx, lm_ggml_tensor * cur, lm_ggml_tensor * idxs, int32_t il) const {
    return lm_ggml_set_rows(ctx, get_score(ctx, il), cur, idxs);
}
