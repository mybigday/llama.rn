...
struct clip_ctx {
    ...
    clip_ctx(clip_context_params & ctx_params) {
        ...
        sched.reset(
            lm_ggml_backend_sched_new(backend_ptrs.data(), backend_buft.data(), backend_ptrs.size(), 8192, false)
        );
    }
...
private:
    ...
    lm_ggml_tensor * build_inp_raw() {
        lm_ggml_tensor * inp_raw = lm_ggml_new_tensor_3d(ctx0, LM_GGML_TYPE_F32, img.nx, img.ny, 3);
    ...
    static lm_ggml_tensor * build_rope_2d(
        lm_ggml_context * ctx0,
        lm_ggml_tensor * cur,
        lm_ggml_tensor * pos_h,
        lm_ggml_tensor * pos_w,
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity) {
    return clip_init(fname, clip_context_params{
        /* use_gpu */   true,
        /* verbosity */ static_cast<lm_ggml_log_level>(verbosity),
    });
}
...
bool clip_model_quantize(const char * fname_inp, const char * fname_out, const int itype) {
    ...
    lm_ggml_type type = static_cast<lm_ggml_type>(itype);

    auto * ctx_clip = clip_init(fname_inp, clip_context_params{
        /* use_gpu */   false,
        /* verbosity */ LM_GGML_LOG_LEVEL_ERROR,
    });
    ...
    for (int i = 0; i < n_tensors; ++i) {
        ...
        lm_ggml_tensor * cur = lm_ggml_get_tensor(ctx_data, name);
    ...
    for (int i = 0; i < n_tensors; ++i) {
        ...
        lm_ggml_tensor * cur = lm_ggml_get_tensor(ctx_data, name.c_str());
        ...
        quantize &= (lm_ggml_n_dims(cur) == 2) && cur->ne[0] > lm_ggml_blck_size(type);
        ...
        LM_GGML_ASSERT(lm_gguf_get_tensor_size(ctx_out, lm_gguf_find_tensor(ctx_out, name.c_str())) == new_size);
        lm_gguf_set_tensor_data(ctx_out, name.c_str(), new_data);
