...
lm_ggml_tensor * clip_graph_pockettts_seanet::conv1d(lm_ggml_tensor * x, lm_ggml_tensor * w, lm_ggml_tensor * b, int stride, int dilation,
    ...
    if (!state_name.empty() && p_total > 0) {
        ...
        lm_ggml_tensor * left = state_in.at(state_name); // [p_total, IC]
        x = lm_ggml_concat(ctx0, left, x, 0);
        state_out.push_back({state_name,
            lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, x, p_total, x->ne[1], x->nb[1],
                                         (size_t) (x->ne[0] - p_total) * x->nb[0]))});
    } else if (pad_replicate && p_total > 0) {
        // the resamplers repeat the first frame instead of zero-padding
        lm_ggml_tensor * first = lm_ggml_view_2d(ctx0, x, 1, x->ne[1], x->nb[1], 0);
        lm_ggml_tensor * left  = lm_ggml_repeat_4d(ctx0, first, p_total, x->ne[1], 1, 1);
        x = lm_ggml_concat(ctx0, left, x, 0);
        x = lm_ggml_pad_ext(ctx0, x, 0, p_extra, 0, 0, 0, 0, 0, 0);
    } else {
        x = lm_ggml_pad_ext(ctx0, x, p_total, p_extra, 0, 0, 0, 0, 0, 0);
    }

    lm_ggml_tensor * y = lm_ggml_conv_1d(ctx0, w, x, stride, 0, dilation);
...
lm_ggml_tensor * clip_graph_pockettts_seanet::conv_transpose1d(lm_ggml_tensor * x, lm_ggml_tensor * w, lm_ggml_tensor * b, int stride,
    ...
    if (depthwise) {
        ...
        lm_ggml_tensor * krn = lm_ggml_reshape_3d(ctx0, w, 1, K, OC);             // [1, K, OC]
        lm_ggml_tensor * xs  = lm_ggml_reshape_3d(ctx0, x, 1, T, OC);             // [1, T, OC]
        col = lm_ggml_mul_mat(ctx0, krn, xs);                                  // [K, T, OC]
        col = lm_ggml_cont(ctx0, lm_ggml_permute(ctx0, col, 0, 2, 1, 3));         // [K, OC, T]
        col = lm_ggml_reshape_2d(ctx0, col, K * OC, T);
    } else {
        lm_ggml_tensor * w2 = lm_ggml_reshape_2d(ctx0, w, K * OC, w->ne[2]);
        w2               = lm_ggml_cont(ctx0, lm_ggml_transpose(ctx0, w2));       // [IC, K * OC]
        lm_ggml_tensor * xt = lm_ggml_cont(ctx0, lm_ggml_transpose(ctx0, x));        // [IC, T]
        col = lm_ggml_mul_mat(ctx0, w2, xt);
    }
    lm_ggml_tensor * full = lm_ggml_col2im_1d(ctx0, col, stride, OC, 0);          // [emit_len + p_total, OC]

    lm_ggml_tensor * out;
    if (state_name.empty() || p_total == 0) {
        out = lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, full, emit_len, full->ne[1], full->nb[1], 0));
    } else {
        // overlap-add the tail the previous call held back
        lm_ggml_tensor * prev = state_in.at(state_name); // [p_total, OC]
        lm_ggml_tensor * head = lm_ggml_add(ctx0, lm_ggml_view_2d(ctx0, full, p_total, full->ne[1], full->nb[1], 0), prev);
        if (emit_len > p_total) {
            lm_ggml_tensor * rest = lm_ggml_view_2d(ctx0, full, emit_len - p_total, full->ne[1], full->nb[1],
                                              (size_t) p_total * full->nb[0]);
            out = lm_ggml_concat(ctx0, head, rest, 0);
        } else {
            out = head;
        }
        state_out.push_back({state_name,
            lm_ggml_cont(ctx0, lm_ggml_view_2d(ctx0, full, p_total, full->ne[1], full->nb[1],
...
lm_ggml_tensor * clip_graph_pockettts_seanet::res_unit(lm_ggml_tensor * x, const clip_seanet::stage & stage, int dilation,
                                                    const std::string & state_prefix) const {
    lm_ggml_tensor * h = lm_ggml_elu(ctx0, x);
    h = conv1d(h, stage.res_conv1_w, stage.res_conv1_b, 1, dilation, false, state_prefix);
    h = lm_ggml_elu(ctx0, h);
    // the second conv is pointwise, it needs no left context
    h = conv1d(h, stage.res_conv2_w, stage.res_conv2_b, 1, 1);
    return lm_ggml_add(ctx0, x, h);
}

lm_ggml_tensor * clip_graph_pockettts_seanet::encode(lm_ggml_tensor * x) const {
    const auto & seanet = model.seanet;

    lm_ggml_tensor * cur = conv1d(x, seanet.conv_in_w, seanet.conv_in_b, 1, 1);
...
lm_ggml_tensor * clip_graph_pockettts_seanet::decode(lm_ggml_tensor * x) const {
    const auto & seanet = model.seanet;
    const bool   stream = !state_in.empty();

    lm_ggml_tensor * cur = conv1d(x, seanet.conv_in_w, seanet.conv_in_b, 1, 1, false,
                               stream ? "dec_in" : "");
