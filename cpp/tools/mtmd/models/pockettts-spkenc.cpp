...
lm_ggml_tensor * clip_graph_pockettts_spkenc::tfm_layer_forward(lm_ggml_tensor * cur, const clip_layer & layer, lm_ggml_tensor * inp_pos, lm_ggml_tensor * kq_mask, int il) const {
...
lm_ggml_cgraph * clip_graph_pockettts_spkenc::build() {
    // the preprocessor hands over the waveform as a single-row "mel", already [n_samples, 1]
    lm_ggml_tensor * inp_raw = build_inp_raw(1);
    lm_ggml_tensor * cur = lm_ggml_reshape_2d(ctx0, inp_raw, inp_raw->ne[0], inp_raw->ne[1]);
    ...
    lm_ggml_tensor * inp_pos = lm_ggml_new_tensor_1d(ctx0, LM_GGML_TYPE_I32, cur->ne[1]);
    lm_ggml_set_name(inp_pos, "inp_pos");
    lm_ggml_set_input(inp_pos);

    // the mimi transformer is causal with a sliding window, see _build_attention_mask()
    lm_ggml_tensor * kq_mask = lm_ggml_new_tensor_2d(ctx0, LM_GGML_TYPE_F32, cur->ne[1], cur->ne[1]);
