...
lm_ggml_cgraph * clip_graph_deepseekocr::build() {
    ...
    if (!is_overview) {
        ...
        LM_GGML_ASSERT(img.ny() >= img.nx());
        LM_GGML_ASSERT(img.ny() % img.nx() == 0);
        ...
        inp_raw = lm_ggml_reshape_4d(ctx0, inp_raw, img.nx() * img.nx(), n_tiles_per_row, 3, n_rows_batch);
        inp_raw = lm_ggml_cont(ctx0, lm_ggml_permute(ctx0, inp_raw, 0, 2, 1, 3));
        inp_raw = lm_ggml_reshape_4d(ctx0, inp_raw, img.nx(), img.nx(), 3, n_tiles_per_row * n_rows_batch);
    }
    ...
    // Building DS-OCR CLIP
    {
        ...
        inp = lm_ggml_reshape_3d(ctx0, sam_out, clip_n_patches, sam_out->ne[2], sam_out->ne[3]);
        ...
        lm_ggml_tensor * cls_embd = lm_ggml_repeat_4d(ctx0, model.class_embedding, n_embd, 1, n_batch, 1);
        inp = lm_ggml_concat(ctx0, cls_embd, inp, 1);
    ...
    sam_out  = lm_ggml_reshape_3d(ctx0, sam_out, sam_out->ne[0], clip_n_patches, n_batch);
    ...
    clip_out = lm_ggml_view_3d(ctx0, clip_out, n_embd, clip_n_patches, n_batch,
    ...
    if (is_overview) {
        ...
        lm_ggml_tensor * imgnl = lm_ggml_repeat_4d(ctx0, model.image_newline, n_dim, 1, h, n_batch);
        cur = lm_ggml_reshape_4d(ctx0, cur, n_dim, w, h, n_batch);
        cur = lm_ggml_reshape_3d(ctx0, lm_ggml_concat(ctx0, cur, imgnl, 1), n_dim, (w + 1) * h, n_batch);
        lm_ggml_tensor * vs = lm_ggml_repeat_4d(ctx0, model.view_seperator, n_dim, 1, n_batch, 1);
        cur = lm_ggml_concat(ctx0, cur, vs, 1);  // (n_dim, h*(w+1) + 1, n_batch)
    } else {
        ...
        cur = lm_ggml_reshape_4d(ctx0, cur, n_dim * grid_x, grid_y, n_tiles_per_row, n_rows_batch);
        ...
        cur = lm_ggml_reshape_4d(ctx0, cur, n_dim, grid_x * n_tiles_per_row, grid_y, n_rows_batch);

        // append newline per row: (n_dim, grid_x*n_tiles_per_row+1, grid_y, n_rows_batch)
        lm_ggml_tensor * imgnl = lm_ggml_repeat_4d(ctx0, model.image_newline, n_dim, 1, grid_y, n_rows_batch);
        cur = lm_ggml_concat(ctx0, cur, imgnl, 1);

        // flatten: (n_dim, (grid_x*n_tiles_per_row+1)*grid_y, n_rows_batch)
        cur = lm_ggml_reshape_3d(ctx0, cur, n_dim, (grid_x * n_tiles_per_row + 1) * grid_y, n_rows_batch);
    }
