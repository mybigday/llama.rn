...
lm_ggml_cgraph * clip_graph_muse_glimmer::build() {
    ...
    lm_ggml_tensor * pos_w    = inp_i32("muse_glimmer_pos_w",    n_tok);
    lm_ggml_tensor * pos_h    = inp_i32("muse_glimmer_pos_h",    n_tok);
    lm_ggml_tensor * sp_perm  = inp_i32("muse_glimmer_sp_perm",  n_tok);
    lm_ggml_tensor * inv_perm = inp_i32("muse_glimmer_inv_perm", n_tok);
    lm_ggml_tensor * ds_perm  = inp_i32("muse_glimmer_ds_perm",  n_tok);

    lm_ggml_tensor * sp_mask = lm_ggml_new_tensor_2d(ctx0, LM_GGML_TYPE_F32, n_tok, n_tok);
    lm_ggml_set_name(sp_mask, "muse_glimmer_sp_mask");
    lm_ggml_set_input(sp_mask);

    // patchify via build_inp (conv2d over raw pixels) + bilinear-resized learned pos-emb
    lm_ggml_tensor * x = build_inp();                                                     // [n_embd, n_tok, 1]
    x = lm_ggml_add(ctx0, x, resize_position_embeddings(LM_GGML_SCALE_MODE_BILINEAR));
    cb(x, "after_posemb", -1);

    // group patches into pgrid x pgrid windows (sparse attention order)
    x = lm_ggml_get_rows(ctx0, x, sp_perm);
    cb(x, "after_sp_perm", -1);

    // per-layer mask: sparse layers get sp_mask, global layers (every sf-th and last) get none
    std::vector<lm_ggml_tensor *> attn_mask_layers(n_layer);
    ...
    x = lm_ggml_get_rows(ctx0, x, inv_perm);
    ...
    x = lm_ggml_get_rows(ctx0, x, ds_perm);                 // [n_embd, n_tok], grouped
    x = lm_ggml_reshape_3d(ctx0, x, n_embd, ds * ds, n_out);// [c, s, o]
    x = lm_ggml_permute(ctx0, x, 1, 0, 2, 3);               // [s, c, o]
    x = lm_ggml_cont(ctx0, x);
    x = lm_ggml_reshape_2d(ctx0, x, n_embd * ds * ds, n_out); // [6144, n_out]
    cb(x, "encoder_out", -1);
    ...
    lm_ggml_build_forward_expand(gf, x);
    return gf;
}
