...
lm_ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (hparams.n_expert_groups > 1 && n_tokens > 0) {
        ...
        selection_probs = lm_ggml_set_rows(ctx0, lm_ggml_scale_bias(ctx0, selection_groups, 0.0f, -INFINITY), selection_probs, expert_groups); // [n_exp_per_group, n_expert_groups, n_tokens]
    ...
    if (norm_w) {
        ...
        if (arch == LLM_ARCH_BAILINGMOE2) {
            weights_sum = lm_ggml_scale_bias(ctx0, weights_sum, 1.0, 1e-20);
...
llm_graph_input_attn_no_cache * llm_graph_context::build_attn_inp_no_cache() const {
    ...
    inp->self_kq_mask = lm_ggml_new_tensor_4d(ctx0, LM_GGML_TYPE_F32, n_tokens, LM_GGML_PAD(n_tokens, LM_GGML_KQ_MASK_PAD), 1, 1);
    ...
    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
        inp->self_kq_mask_swa = lm_ggml_new_tensor_4d(ctx0, LM_GGML_TYPE_F32, n_tokens, LM_GGML_PAD(n_tokens, LM_GGML_KQ_MASK_PAD), 1, 1);
